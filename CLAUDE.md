...
**Remaining stages, and one departure from the issue's list.** The issue's stages 1 and 2 (parser; unit tests with fixtures) landed together — a parser without its tests is not a stage. Next: the mutation (stage 3, taking an upload and returning the `ParsedRoster` as a preview *without writing*, then a confirm that writes it — the CSV path's admission call `_admit_late_racers` once per batch (#343) and its `CHECKIN` classification are the precedent, since an import is desk work); the modal (stage 4, beside **Import from CSV** in the Add Racer split button); the docs (stage 5, `race-setup.md` and the comparison page). Duplicate prevention *against the roster already in the race* is a stage-3 question — it needs the database — and `duplicate_number_problems`'s docstring says so rather than growing a second copy of the rule client-side.
