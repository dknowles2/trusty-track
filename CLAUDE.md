...
**`Install Playwright system libraries` is 30–41 seconds per Playwright job even on a cache hit**, and is the largest single non-test cost in the workflow. The browser binary is cached; the system packages it links against are installed every time.
