...
**The backend suite runs `-n auto`, and neither Playwright job is sharded any more.** A pull request used to wait about three and three-quarter minutes; the three longest jobs were the two Playwright runs and the backend suite, and none had a single slow part to fix — they are 5,900 tests and 70 browser specs. The backend suite is split across processes, which depends on **nothing in it sharing state between tests** (in-memory database per test; the data directory is per xdist worker, which `conftest.py` had to be taught). The Playwright browser is cached on its version — 150 MB that never changes between runs, and a third of each of those jobs.

**Both Playwright suites are parallel *within* one run instead of split across two machines**, which is strictly better: no second server, no second setup phase. Sharding is also incompatible with how they are built now — both configs use **project dependencies**, and Playwright re-runs a dependency project in full in every shard. The `End-to-End` and `Doc Screenshots` gate jobs still carry the names the ruleset requires, so the jobs behind them can be renamed or re-split without anybody editing repository settings.
