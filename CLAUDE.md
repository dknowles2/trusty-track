...
| [`ops.md`](.claude/rules/ops.md) | `services/backup.py`, the backup endpoints, HTTPS/`TRUSTYTRACK_HTTP_ONLY`, `deploy/raspberry-pi/`, `scripts/install-pi.sh`, or the first-run gate | Backup and restore · Networking: HTTPS by default, plain HTTP as an opt-out · The pre-built Raspberry Pi image (stage 1 of #724) · First-run gate |
