...
**`Terminology` is served resolved on `Race` and on `InitialConfigStatus`** (the screens with no race — Home, System Settings), and the raw four override columns are served alongside it on `Race`, `Organization` and `InitialConfigStatus` so an edit form can tell "inherited" apart from "set explicitly" — a form seeded from the resolved words could never offer to clear them, the same trap noted under Awards for a `SPEED` award's recipient.

**No consumer reads the resolved words yet.** This stage is storage, resolution and the two settings screens; the frontend keeps saying "Den" and "Pack" everywhere else until stage 4 wires up a `TerminologyContext` and rewrites roughly fifty files. The ballot and the audience displays are checked by hand once that lands, not swept mechanically.
