...
`domain/roster_import.py` is the vocabulary, `domain/gprm.py` the GrandPrix Race Manager mapping, `services/importers/` the files ([#618](https://github.com/dknowles2/trusty-track/issues/618), stage 1 of 5). A pack switching from GPRM arrives with years of rosters, car numbers and den assignments in its database, and the only door in was the CSV importer — export tables by hand, reshape the columns. This stage is the parser and its rules only: **nothing calls it yet**. The mutation, the upload-preview-confirm modal and the user docs are the later stages, and `docs/race-setup.md` deliberately does not mention it until a screen exists.

**`ParsedRoster` is the seam every importer plugs into, and [#661](https://github.com/dknowles2/trusty-track/issues/661) (DerbyNet) is queued to use it.** `ImportedGroup`, `ImportedRacer` (fields mirror `schemas.RacerCreate`, `group` names a group by *name*), `ImportProblem` (a sentence, `blocking` or not — `csvMapping.ts`'s `line === 0` as one field) and `RosterImportError` (the file is not readable at all). A parser returns the value; the preview, the confirmation and the write path — when they land — are built against it once, not per program. DerbyNet's file is the *same table family* (`RegistrationInfo`, `Classes`, `Ranks`), so #661 is expected to be a second thin entry point in `services/importers/` over the same `roster_from_tables` and the same `SqliteTables`, with a DerbyNet-only rule or two (its `Partitions` table is the finest grouping there) rather than a second mapping. `duplicate_number_problems` is the one home for the in-file duplicate rule (#60's), and it is in the shared module for that reason.
...
**Stage 3, the mutation, is preview-then-confirm, and both re-parse the same upload rather than trusting a session on the server.** `previewGprmImport(raceId, fileData)` writes nothing and returns a `GprmImportPreview` (`groups`, `racers`, `problems`, `canImport`) — `fileData` is a base64 data URL, the same shape `uploadImage`'s `dataUrl` already takes. `confirmGprmImport(raceId, fileData)` decodes and parses the identical bytes a second time and writes them through `crud.write_imported_roster` (the one door: an existing racing group is matched by name and reused, same as `importRacers`' CSV loop; a new one is created grey, `#808080`, for the same reason that loop's auto-created groups are), then calls `_admit_late_racers` once for the whole batch — #343's fix, since a GPRM roster can carry already-checked-in racers (`PassedInspection`) and per-racer admission would regenerate an unraced round once per racer over a desk queue — and publishes `RaceChangeKind.ROSTER`. There is deliberately no server-side session holding the upload between the two calls: re-sending the (small — GPRM keeps photos as separate files, never in the database) file on confirm means what gets written can never drift from what the preview showed, at the cost of decoding it twice. `sqlite3` opens files, not buffers, so both calls write the decoded bytes to a temporary file (`_parse_gprm_upload`, removed in a `finally`) before `parse_gprm_database` sees them — the same shape `services/backup.py`'s restore uses for an uploaded archive. `RosterImportError` is re-raised as a plain `ValueError`, so a file the parser refuses reaches the operator the same way `importRacers`' "Race not found" does.

**Duplicate prevention against the roster already in the race is `domain.roster_import.existing_number_problems`**, the pure counterpart to `duplicate_number_problems` that the docstring on the latter promised: it takes `{car_number: "First Last"}` as data rather than a session, fed by `crud.existing_car_number_holders` — the query half stays in `crud`, same rule/I-O split as everywhere else. Only the *first* file holder of a number is checked against the existing roster; a later file holder sharing that number is already reported by `duplicate_number_problems` against the first, and checking it again here would say the same collision twice. The preview's `problems` list is the in-file duplicates followed by these — the reader has no reason to care which rule found a given line.

**Classified `CHECKIN`, the same bucket as `importRacers` and for the same reason: importing a roster is desk work**, not scheduling or a race's own settings — a pack that registers everyone by spreadsheet and then walks in with their old GPRM file at the check-in table should not need to find the operator. `confirmGprmImport` is in `race_lock.py`'s `LOCKED_MUTATION_RESOLVERS` (`_direct_locked`, same as `importRacers`) since it writes racers into a race; `previewGprmImport` is deliberately absent there, since it writes nothing and is exactly the kind of read-shaped mutation the module's own docstring says stays reachable on a locked race. The demo's denylist (`api/demo_policy.py`) refuses `confirmGprmImport` for the same reason it refuses `importRacers` — bulk row creation from caller-supplied data, one visitor's file ruining the demo for everyone else until the reset — and leaves `previewGprmImport` reachable, since parsing an upload into a temporary file that is deleted before the response returns costs the demo nothing an ordinary read does not.

**Stage 4, the modal.** `GprmImportModal.tsx` sits beside **Import from CSV** in the roster's Add Racer split button. Unlike the CSV path there is no client-side column mapping — the file is a database, not a spreadsheet, and the mapping already happened in `domain/gprm.py` — so the modal's job is: pick a file, read it as a data URL (`FileReader.readAsDataURL`, no server round trip until the mutation), call `previewGprmImport`, render the groups/racers/problems it comes back with, and call `confirmGprmImport` with the same data URL on **Import**. A file the parser does not recognise — an Access `.mdb`, a CSV, anything else — surfaces `previewGprmImport`'s `ValueError` as the sentence the parser wrote (the Access refusal names the fix: open it in a current GPRM, which re-saves as SQLite), not a generic "could not be read."

**What #661 (DerbyNet) still needs to add, on top of this seam.** `roster_from_tables` already treats a DerbyNet-shaped file correctly for the *shared* table family (`_read_classes` skips DerbyNet's aggregate classes on sight, per its own docstring) — #661's own work is a second thin entry point in `services/importers/` (its own `RosterImportError` messages, since a DerbyNet file has no pre-v18-Access equivalent to refuse) and, per `domain/gprm.py`'s docstring, a DerbyNet-only mapping rule or two (its `Partitions` table is a finer grouping this schema does not have). Stage 3's mutation shape needs nothing new: a second `preview<Source>Import`/`confirm<Source>Import` pair (or a shared pair taking a `source` argument, if the two parsers turn out to need identical arguments — a decision left to #661, since GPRM's is the only shape known today) calls the same `_parse_gprm_upload`-shaped helper over the new parser, the same `crud.write_imported_roster`, and the same `existing_number_problems`. Nothing here is GPRM-specific below `services/importers/gprm.py`.
