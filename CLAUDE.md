...
`domain/terminology.py`, storage on `Organization` and `Race` ([#496](https://github.com/dknowles2/trusty-track/issues/496), stage 3). An AWANA club, a school science fair or a 4-H group runs the same event format Trusty Track was built for, and had to read "Den" and "Pack" on every screen with no way to change either — the word was the column name. Two configurable terms fix that: what a racing group is called, and what the organization itself is called, each stored as a singular and a plural because English plurals are irregular and deriving "Classes" from "Class" is a rule nobody should own. A racing group's `division` (the field stage 2 turned into free text — Cub Scout rank, school grade) stays a fixed "Category" label rather than becoming a third term: it is branding on one row, not vocabulary a whole screen is built from.
...
**Eight nullable columns, not a blob and not a key-value table.** A blob is the shape [#5](https://github.com/dknowles2/trusty-track/issues/5) spent a release removing; a key-value table is stringly typed, and a misspelled key would sit there reading as an override and doing nothing, invisible to both mypy and the schema. Two terms across two scopes is eight columns — four on `Organization`, four on `Race` — and each one checkable forever.
