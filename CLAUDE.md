...
| [`race-day-ui.md`](.claude/rules/race-day-ui.md) | `raceFlow.ts`, `heatSession`, `RaceExecution.tsx`, `RaceControl.tsx`, shortcuts or the chime | Race-day keys and the finish sound · What is on the track right now · What the operator screen does between heats · The action row, the preferences popover, and the phone layout |
...
| [`ops.md`](.claude/rules/ops.md) | `services/backup.py`, the backup endpoints, HTTPS/`TRUSTYTRACK_HTTP_ONLY`, `services/discovery.py`/mDNS, `deploy/raspberry-pi/`, `scripts/install-pi.sh`, or the first-run gate | Backup and restore · Networking: HTTPS by default, plain HTTP as an opt-out · Finding the instance: mDNS · The pre-built Raspberry Pi image (stage 1 of #724) · First-run gate |
| [`ci.md`](.claude/rules/ci.md) | `.github/workflows/`, the release workflow, a branch or worktree, or `frontend/e2e/functional/` | What CI checks · Every issue carries a label · A branch is deleted when its pull request merges · What the functional e2e specs are for |
...
| `SchedulingStrategy`   | `GENERAL`, `ELIMINATION`, `BALANCED` — a round's *format*, not the algorithm that builds a `GENERAL` round's schedule |
| `SchedulingAlgorithm`  | `PPC`, `ROTATION`, `PERFECT_N` — the algorithm behind a `GENERAL` round's schedule (`Round.algorithm`, nullable, null means `PPC`); registry in `domain/schedulers/__init__.py`, `.claude/rules/scheduling.md`'s "Heat scheduling (PPC)" ([#1090](https://github.com/dknowles2/trusty-track/issues/1090)) |
...
**Migrate a second database from a test with `backend/tests/helpers.py::migrate_to_head`, not a subprocess.** It is `init_db()` itself — same legacy detection, same stamp, same upgrade — pointed at an engine you hand it, which is what `init_db(target)` exists for. Shelling out was the old way, because `database.py`'s `engine` is module-level and resolved from the environment at import time, so a second database meant a second interpreter; that cost about 0.9s per call against roughly 0.3s of actual migrating, once per test across four files, and it made them the four slowest in the suite. `helpers.run_alembic` still shells out and should: it is the CLI, with its own argument parsing and its own refusal guard, and that is what `test_migration_cli_guard.py` is about. Arrangement is not the subject — seed a fixture in-process even there.
...
- **A subscriber's queue is bounded, and a full one drops its oldest entry — for a channel whose payload is a full snapshot, never a delta** (`api/pubsub.py`'s `MAX_QUEUE_SIZE`, [#776](https://github.com/dknowles2/trusty-track/issues/776)). Safe there because a subscriber that missed several snapshots and gets only the newest one is *correct*, since the newest already supersedes them all. `_PubSub.publish` therefore never blocks: a caller is a mutation resolver or `TimerManager` recording a result mid-heat (#9), and a display nobody is reading must not be able to stall either one. A subscriber that has fallen `MAX_QUEUE_SIZE` snapshots behind is already the half-open connection `api/liveConnection.ts`'s `pingWatchdog` is about to close — this bound only keeps memory flat for the window before that happens, it does not replace the watchdog. **Two subscribers don't fit that premise, and each opts out on its own `subscribe` call** (`drop_oldest_when_full=False`), because the flag is per-subscriber rather than per-channel. `display_assignment:{id}` ([#881](https://github.com/dknowles2/trusty-track/issues/881)): its payload is always `None`, and the row it wakes a subscriber to re-read carries a `slide_delta` that is *overwritten* per `advanceDisplay` call rather than accumulated (`.claude/rules/displays.md`'s "Identify is an event, not a state") — so dropping an older wake-up in favour of a newer one destroys an operator's step rather than superseding it with a fresher one. `raceStateChanged` on `race_state:{race_id}` ([#896](https://github.com/dknowles2/trusty-track/issues/896)): every other subscriber on that channel (`heatSession`, `leaderboard`, `onDeck`, `currentlyRacing`, `timingStats`, `heats`, the free-race pair) re-reads the database on any wake-up and fits the premise fine, but `raceStateChanged` forwards its `RaceStateChangedEvent` straight to the client for the normalized cache to merge (#12) — a `HEAT_RESULT`/`RACER` payload is a genuine per-entity diff, not a disposable snapshot, so dropping an older one loses it outright rather than superseding it. Reachable (`test_race_state_changed_queue_drop.py`), though it had self-healed on the next structural event often enough to go unreported. Check any new subscription against this before assuming the default bound is safe for it.
- **The domain layer (`backend/domain/`) imports no SQLAlchemy and no Strawberry.** Scheduling, scoring, and advancement are plain functions over plain values; `crud.py` and `services/` load rows, call them, and persist the answer. Put a *rule* there and its *I/O* in the caller. Keep it importable without a database — that is what lets `test_domain_scheduling.py` run every racer count from 2 to 20 against every lane count from 2 to 8 in under a second, which is how issue #26 was found. `backend/tests/test_format_crossings.py` is the same sweep shape one layer up, through the database and the GraphQL client (#1025, #1052): general round style × championship shape × master running order, raced to the end, holding heat-number uniqueness, the "nobody races alone" rule and `Race.status` reaching `FINISHED` — it is what found #1054.
