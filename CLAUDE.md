...
The normalized `heat_lanes` table exists and is kept current. Its rows are built from the lane *values* a writer supplied: `crud.set_heat_lanes` — the one door — stages them on the instance, and `backend/db/lane_sync.py` writes them on `after_flush`, which is when a newly created heat first has an id. `lane_results` is written alongside as a **derived** column, so a rollback has data and the readers can move one at a time. A malformed blob can no longer reach the table; parsing one survives only as a logged fallback for a write that skipped the door, which the AST guard in `test_heat_lanes_write.py` says cannot happen — that guard scans the whole backend package, because a narrower one scanning only `crud.py` could hold the property only where it already held, and there was a second writer outside it. **Making the table authoritative is the remaining half of #5, tracked as #72 — and step 5, dropping the column, is deliberately not done.** There has been one release, `v1.0.0`, and every commit of this work is after it, so nothing carrying the flip has shipped; the blob is the only way back for a database upgraded from `v1.0.0`, and dropping it also retires the `lanes_out_of_sync` net that validated every step. Ship a release, put it through an event, then revisit. `lanes.carry_extras` stays for the same reason — it preserves keys that pre-normalization blobs carry, which is the population being upgraded, not a hypothetical. **What is left of it is deletion, not performance.** The query-shaped wins #5 predicted have either landed or turned out not to be worth taking: the N+1 is already gone — `crud.lanes_for_heats` is one query for a whole set of heats — and pushing the aggregation further into SQL would move the `TIMED`/`POINTS` rules, DNF penalty and all, out of `domain/` and into a query, against #8. Measured on a 200-racer, 200-heat race, the leaderboard is 12 queries and the round-completion check 3, both flat in heat count and both single-digit milliseconds; there is nothing there worth trading the domain layer's purity for. The racer foreign key does enforce on write now, and carries `ON DELETE SET NULL` — see below. Two consequences:
