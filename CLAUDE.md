...
- **A subscriber's queue is bounded, and a full one drops its oldest entry** (`api/pubsub.py`'s `MAX_QUEUE_SIZE`, [#776](https://github.com/dknowles2/trusty-track/issues/776)). Safe only because every payload published here is a full snapshot, never a delta — a subscriber that missed several and gets only the newest one is *correct*, since the newest already supersedes them all. `_PubSub.publish` therefore never blocks: a caller is a mutation resolver or `TimerManager` recording a result mid-heat (#9), and a display nobody is reading must not be able to stall either one. A subscriber that has fallen `MAX_QUEUE_SIZE` snapshots behind is already the half-open connection `api/liveConnection.ts`'s `pingWatchdog` is about to close — this bound only keeps memory flat for the window before that happens, it does not replace the watchdog.
