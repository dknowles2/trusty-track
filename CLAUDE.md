...
**What this stage does not touch: which heat the operator screen offers to run next.** `RaceControl`'s `sortedHeatsEx` still orders every heat `(roundNumber, heatNumber)` — round-number primary — and `ScheduleManagement`'s own per-round Run buttons still disable a round until the one before it (by round number) is finished. Interleaving `heatNumber` across rounds does not change either of those, so today the master order is the announcer's paper view and the operator's own manual navigation between rounds, not yet something the Race-day screen enforces or assists. Closing that gap means teaching the execution flow's own ordering about `heatNumber` spanning rounds, which is a separate, riskier change to code three other features already depend on (`raceFlow.ts`, the round-summary edge detector, the on-deck/currently-racing subscriptions) — flagged as a follow-up rather than folded in here.
