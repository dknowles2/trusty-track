...
rather than a fallback. Choosing **Timed** instead on a no-timer track is
right only for a volunteer typing a stopwatch reading by hand — for a pack
with no timing device at all, calling the finish by eye, it leaves no
**Place** column to enter an order into, and the race form says so at the
moment Timed is chosen ([#1324](https://github.com/dknowles2/trusty-track/issues/1324)).
...
- A typed-in place has to make sense: **1st or better**, no higher than the
  number of cars in the heat, and not reused for two cars unless their recorded
  times genuinely match (#816). Trusty Track refuses the save and says why,
  rather than storing a number that would score wrong — a `0` or a negative
  place would otherwise *subtract* from a car's total.
