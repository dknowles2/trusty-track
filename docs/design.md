...
-   Manual advancement (#711): `pinRoundField(raceId, roundId, racerIds)` rebuilds a championship round for exactly the given racers and sets `Round.fieldPinned`, which the recorded-result cascade and `populate_round_if_decided` both then leave alone rather than re-fielding from the standings; refused for a general round, for a round already raced, and for a pick that is too short, duplicates a racer, or names anyone not checked in (#228). `unpinRoundField(raceId, roundId)` clears the flag and hands an unraced round back to the standings as they stand now. `AdvancementStatus.fieldIsPinned` reports which is in effect, reading the pinned lanes directly rather than the computed pick when set.
