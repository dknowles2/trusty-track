...
-   Data: `importRacers` (CSV), `previewGprmImport(raceId, fileData)` / `confirmGprmImport(raceId, fileData)` (a GrandPrix Race Manager database, #618 — `fileData` is a base64 data URL like `uploadImage`'s; preview parses and returns a `GprmImportPreview` without writing, confirm re-parses the identical upload and writes it, so nothing is trusted back from the client between the two calls; both `CHECKIN`, matching `importRacers`), `uploadImage` (base64), `populateRace` (test data), `createPracticeRace` (a whole rehearsal event on a fake timer; resumes the most recent one rather than building another unless `startNew` is set, #588)
