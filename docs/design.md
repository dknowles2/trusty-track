...
-   **Roster import from other derby programs:** `backend/domain/roster_import.py` is the shared result type (`ParsedRoster`: groups, racers, problems) every importer returns; `backend/domain/gprm.py` maps GrandPrix Race Manager's `RegistrationInfo`/`Classes`/`Ranks` tables onto it over a plain-rows `TableSet` protocol; `backend/domain/derbynet.py` (#661) reads the same table family through `gprm.roster_from_tables` and adds one thing GPRM's schema does not have, DerbyNet's own `Partitions` table; `backend/services/importers/` owns the files (read-only SQLite, header sniffing, the refusal sentences) for both. See CLAUDE.md's "Importing a roster from another derby program".
...
-   **`Track`**: Configuration of the physical track.
    ...
    -   `scale_ratio` (`float`, `NOT NULL`, default `25` — #610 stage 2) — the vehicle-to-real-life ratio scale speed is computed against (`domain.scale_speed.scale_mph`); a fact about this track's cars, not a global constant, since a Space Derby or Raingutter Regatta track is not built to 1:25
    -   `show_scale_speed` (`bool`, `NOT NULL`, default `true` — #610 stage 2) — whether scale speed is offered on this track's surfaces at all. A plain flag, not folded together with `length_feet > 0`: readers that decide whether to actually display a speed AND this with a positive `length_feet` rather than treating the flag alone as sufficient
...
-   **`Race`**: Specific race event instance.
    ...
    -   `one_trophy_per_racer` (Boolean, `NOT NULL`, default `false` — #615) — at most one trophy per racer: a racer who already holds an award is skipped when resolving a later one, so a den speed trophy rolls down to the next fastest car once its own winner has already taken the pack championship. Computed fresh on every read, like every other award (`domain/roll_down.py`); nothing about who holds what is stored. Off by default, so an upgraded install resolves every award in isolation exactly as it always has.
    ...
    -   `display_theme`, `printables_theme` (`varchar`, nullable — #1081) — a per-race override of the organization's Display/Printables themes, for the pack running an ordinary derby one weekend and a themed one the next. Null means inherit the install's setting — *unlike* `Organization`'s own pair, here null and `"MATCH_APP"` are different answers (inherit vs. an explicit override pinning this race to Field Uniform regardless of what the install picks later), so `clearDisplayTheme`/`clearPrintablesTheme` on `updateRace` are the explicit way back to null, following `clearNameDisplay` above. `domain.theme.resolve_theme_setting(organization_setting, race_override)` is the one place the two layers resolve; `Race.resolvedPrintablesTheme` serves the answer for the four print pages, and the `displayAssignment` subscription's own `_display_theme_setting` resolves the Display side directly against the database (see the subscription entry below).
...
-   **`Racer`**: Participant details.
    ...
    -   `racer_image_original_url`, `car_image_original_url` (optional, #1241) — the photo `racer_image_url`/`car_image_url` was cropped from, if any. Null for every racer until the first Rotate / Recrop, camera capture, or bulk photo assignment on that side; that racer's Rotate / Recrop opens on the current image itself, exactly as it did before this pair existed. `updateRacer`'s `clearRacerImage`/`clearCarImage` null the matching original and edit alongside the derived image; `bulkAssignPhotos` nulls both for the side it assigns, since an assigned file was never cropped through this app.
    -   `racer_image_edit`, `car_image_edit` (optional, #1241) — JSON the server never interprets: `{"rotation": 0|90|180|270, "crop": {"x", "y", "width", "height"}}`, the crop expressed in the *rotated* image's own pixel space — exactly the state the frontend's crop tool (`ImageCropModal`) already holds, so reseeding it from a stored edit needs no coordinate conversion.
    -   `home_unit` (`varchar`, nullable — #1076 stage 1) — a racer's own unit, e.g. "Pack 12", free text and distinct from `racing_group_id` (the rank they race within, which at a district event is shared across units). `None` on every racer at an ordinary single-pack race, and every surface that renders it is conditional on that, so nothing there changes shape. `Race.homeUnitLabel` resolves what to call it — "Home " plus the organization's own word, resolved against the organization layer only, deliberately never this race's own override of that word (a district event still calls it "Home Pack", not "Home District") — see `domain/terminology.py`, CLAUDE.md's Terminology section.
-   **`Round`**: A collection of heats within a race (e.g., "Wolves Round", "Championship").
    ...
    -   `field_pinned` (Boolean, default `false`) — a championship round's line-up was chosen by hand rather than computed (#711). `pinRoundField(raceId, roundId, racerIds)` rebuilds the round for exactly those racers (at least two, no duplicates, all checked in — #228) and sets the flag; while set, the recorded-result cascade (`invalidate_future_rounds`) and `populate_round_if_decided` both leave the round alone instead of re-fielding it from the standings, which is what lets a hand-picked field survive the next preliminary result. `advancement_source`/`advancement_num_racers` are untouched and still describe the suggestion the pick started from; `advancement_num_racers` is not enforced against a pin. `unpinRoundField(raceId, roundId)` clears the flag and, if nothing has been raced, re-fields the round from the standings as they stand now; a raced round only loses the flag, and a hand-picked field that no longer matches who would advance today shows the ordinary **Line-up out of date** badge (`AdvancementStatus.fieldIsStale`, #229).
    ...
    -   `runs_per_lane` (optional, #1088) — how many heats a car races per lane, the round wizard's own answer; a parameter to `generate_heats_for_round` until this column existed, so a round created before it reads back null, treated the same as `1`. Exists so `Race.roundPlan` (below) can answer "how many runs" for a round without also knowing the field size its heat count was generated against.
...
-   **`HeatReplay`**: One camera's stored replay clip for one heat (#177 stage 2 — `table heat_replays`).
    -   `id` (PK), `heat_id` (FK to Heat, `ON DELETE CASCADE`), `camera_id`, `recorded_at`, `path`, `duration_ms`, `t0_offset_ms`, `size_bytes`, `created_at`
    -   Written **only** when `Organization.keep_replays` was on at the moment the clip landed (`POST /replay/`, `services.replays.record_stored_clip`) — never retroactively — so an empty `Heat.replays` list and the setting being off are the same fact told two ways. Off (the default), a clip is never persisted here at all: stage 1a's in-memory `ReplayStore` and its delete-after-next-heat rule are the whole story, unchanged.
    -   `recorded_at` is the same string `Heat.recorded_at` held at capture time — a heat re-run while the setting is on keeps *both* rows, since a corrected result's clip is history worth keeping alongside the run it replaced, not a second heat competing for a retention budget counted by `heat_id`.
    -   Trimmed by `services.replays.enforce_retention`, applied once per upload against `Organization.replay_retention_heats` (last-N-heats) and `replay_retention_mb` (a total-size cap) — both nullable and independent; both null means genuinely unbounded. `domain.replays.heats_beyond_count`/`heats_beyond_size` are the pure rules underneath it.
    -   The **row** is cascade-deleted with its heat; the **file** under `DATA_DIR/replays/` is not (SQLite cannot delete one for us) — `services.replays.discard_clips_for_race` removes it explicitly, read before `crud.delete_race` removes the heats naming it. `GET /replay/<name>` now resolves through `ReplayStore.known_filename` before ever touching the filesystem, rather than `is_relative_to`/`is_file()` alone — the stage 1a review's own "worth revisiting with longer retention" note.
    -   Rebuilds `ReplayStore`'s in-memory index at startup when `keep_replays` is on (`ReplayStore.rebuild_from_db`), replacing stage 1a's unconditional sweep for that case; a restore always sweeps and also deletes every row outright, since the archive never carries `replays/` itself (`ops.md`'s backup section) and a surviving row would name a file that never made the trip.
-   **`Award`**: A trophy, and where its recipient comes from (#170).
    ...
    -   The GraphQL `Award` type carries three further fields resolved from `Race.one_trophy_per_racer`'s roll-down (#615, `domain/roll_down.py`, `services/awards.resolutions_of`), none of them stored: `position` (the 1-based row the recipient actually held in the award's own narrowed standings — equal to `place` when nothing rolled, greater when it did); `passedOver` (every racer ranked above `position` who was skipped because they already held a trophy on another podium, best-first, each carrying the racer and the award that already named them — the explanation for a rolled-down trophy); and `duplicateOf` (set on a **judged** award only: the award its chosen racer already holds — a computed rule never displaces a person's choice, so the judged award keeps its racer regardless and this is only the signal for the screen to warn about the collision). All three are the isolated per-award answer (no provenance, `duplicateOf` always null) whenever `one_trophy_per_racer` is off.
...
-   **`Scene`**: A named, operator-created recipe for what every audience display should show at once (#613).
    -   `id` (PK)
    -   `race_id` (FK to Race, `ON DELETE CASCADE`)
    -   `name`, unique per race (`uq_scene_race_name`)
    -   Deliberately **stored**, unlike a `Display`'s own live assignment (in-memory only, `services/displays.py`) — a scene is something the operator spent real time composing and wants to survive a server restart mid-event. The four *built-in* presets (Check-In, Racing, Intermission, Awards) need no table at all: they are code (`domain/scenes.py`), applied live against whichever displays happen to be connected, not something an operator created.
-   **`SceneAssignment`**: One display's entry within a `Scene` (table `scene_assignments`).
    -   `id` (PK), `scene_id` (FK to Scene, `ON DELETE CASCADE`), unique on (`scene_id`, `display_id`)
...
-   `race(raceId)` — Get a single race with nested `racers`, `racingGroups`, `rounds`, `heats`, `leaderboard`, `awards`. Carries `votingOpen` (#305) and `oneTrophyPerRacer` (#615); each `Award` carries `votable` and `voteTally` (ranked `(racer, voteCount)` pairs, from `services/awards.vote_tallies_for` — one query for the whole race, not one per award), and `position`/`passedOver`/`duplicateOf` — the roll-down's own provenance for `oneTrophyPerRacer`, from `loaders.award_resolutions`, the same whole-race memoisation `award_recipients` already used. Carries `terminology` — this race's override layered over its organization's default (`domain/terminology.py`, #496 stage 3; #551 adds the vehicle term) — plus the raw `racingGroupSingular`/`racingGroupPlural`/`organizationSingular`/`organizationPlural`/`vehicleSingular`/`vehiclePlural`/`vehicleArtworkKey` override fields the edit form reads back. Also carries `homeUnitLabel` (#1076 stage 1) — "Home " plus the resolved organization word, but resolved against the organization layer alone, never this race's own override of it, so a district event's races still call a racer's own unit "Home Pack" rather than "Home District".
-   `racers(raceId, skip, limit)` — List racers.
-   `auditLog(raceId, limit, beforeId, categories, noteworthy)` — The activity timeline, newest first. Operator-only, and it enforces that itself: the role policy guards mutations, and this is the query a wall display must never be able to run (#219). `categories` (a list of `AuditCategory` — `RESULTS`, `SCHEDULE`, `ROSTER`, `AWARDS`, `DISPLAYS`, `SETUP`) and `noteworthy` narrow the timeline server-side, before `limit`/`beforeId`, so paging under a filter never skips a matching older row (#1253). Each entry also carries its own `category`, resolved the same way.
-   `racer(racerId)` — Get a single racer.
-   `tracks()` — List configured tracks.
-   `organizations()` — List organizations.
-   `initialConfig()` — Initial configuration status (organization + track). Carries `terminology` (the organization's default words, resolved) and the raw `racingGroupSingular`/`racingGroupPlural`/`organizationSingular`/`organizationPlural`/`vehicleSingular`/`vehiclePlural`/`vehicleArtworkKey` overrides the settings form reads back (#496 stage 3; #551 adds the vehicle term and its picture).
-   `rounds(raceId)` — List rounds for a race.
-   `advancementStatus(raceId, roundId)` — Check round advancement eligibility.
-   `raceStats(raceId)` — Lane fairness, per-racer aggregates, top moments, racing group comparison, and the track's all-time records (`trackRecords`: the fastest cars across every race on this race's track, computed on read by `services/records.py`). Carries `topScaleMph` (#610 stage 4) — the fastest-heat highlight's time converted through `domain.scale_speed.scale_mph`, null under the track's own scale-speed settings.
...
-   `scenes(raceId)` — A race's saved display scenes (#613), oldest first. Unlike `displays`, this **is** a database read — a scene is stored, not presence.
-   `scenePresets()` — The four built-in scene recipes (Check-In, Racing, Intermission, Awards), each a key and a label, read off `domain/scenes.PRESETS` rather than hardcoded on the frontend.
...
-   `schedulingAlgorithms(racerCount, laneCount)` — Every registered `GENERAL`-round scheduling algorithm (#1090, part D), for the round wizard's "How heats are built" choice: `value`/`label`/`guarantee`/`absorbsLatecomer` straight off the `domain/schedulers` registry, plus `unavailableReason` and `heatCount` computed against the exact field/lane shape asked for — both null for PPC and Lane rotation at any shape, and real answers for Perfect-N, the one algorithm whose availability and heat count actually depend on the shape. In registry order, PPC (the default) first.
-   `heatSession(trackId, heatId)` — What is on the track right now (see below).
-   `heatReplay(raceId)` — A one-shot read of the same answer the `heatReplay` subscription gives (see below), for a caller that wants to poll rather than hold a socket open. Added for the functional e2e suite's `instantReplay.spec.ts` (#1205), which used to watch the camera page's own upload request over the Vite dev proxy for up to 90s — a wait that, under CI contention, held the proxy alongside every other spec's ordinary requests. Polling this field goes straight to the backend instead.
...
-   Race: `createRace` (since #662 also takes `racingGroups`, created in the same transaction as the race, and the seven per-race terminology override fields, so the setup wizard's answers land in one mutation rather than a race followed by N group inserts; since #722, `awards` — copied award definitions, never a recipient; since #1088, `roundPlan` — a copied round plan, built through `crud.create_rounds_from_plan` in the same transaction, with a copied `ROUND:<id>` award in `awards` re-pointed at whichever of the new race's own rounds the plan reproduces), `updateRace` (absent means leave alone throughout, so a per-race terminology override — racing group, organization, and, since #551, vehicle — takes `clearTerminology` to get back to inheriting — the same shape as `clearWeightLimit`, #496 stage 3; a per-race name-display override, #552, takes `clearNameDisplay` the same way, since `"FULL"` set explicitly is a real override there rather than the null state; a per-race Display/Printables theme override, #1081, takes `clearDisplayTheme`/`clearPrintablesTheme` the same way, since `"MATCH_APP"` set explicitly is a real override there too — and publishes to the displays already on this race (`_publish_race_display_theme_change`) when either changes; `isLocked` toggles the lock, #585 — while the race is locked, this mutation itself is refused unless the payload touches nothing else, see `api.race_lock`), `deleteRace` (stays reachable on a locked race — see `api.race_lock`)
...
-   `seedChampionshipAwards(raceId)` (#1082): the Awards page's empty-state "Add the N championship trophies" button. Creates one `SPEED` award per place `1..Race.championshipTrophies`, sourced from the race's own final round (`ROUND:<final id>`), or one set per racing group when the final's own `advancementSource` is `EACH_GROUP`. The same seeding `createRoundWizard` and `createRound` already do the moment a final round is created; this is the by-hand door for a race whose wizard was skipped or whose seeded set was deleted. A no-op — returns `[]` — whenever the race already has a `SPEED` award (presence is the whole guard) or has no championship round yet, in which case it raises instead.
...
-   Audience displays: `assignDisplay`, `advanceDisplay`, `identifyDisplay`, `renameDisplay`, `forgetDisplay` (operator-only — a display is a `VIEWER` and is *told*, never asks) (takes the whole set of out-of-service lanes, since the screen is a row of checkboxes and a repaired lane is simply absent; brings existing scheduled heats into line). `identifyDisplay(displayId)` bumps `Display.identifySeq` (#495) — the same step-not-state shape as `advanceDisplay`'s `slideSeq`, so the screen flashes its own name once rather than on every reconnect. `assignDisplay` also takes `replays` (#177 stage 1a) — whether this screen plays a heat's clip after its results overlay, the same omitted-means-keep-current rider shape as its other booleans.
-   `setCameraTrack(displayId, trackId)` (#177 stage 1a): tells a `CAMERA`-role display which track's timer it should listen to for its clip boundaries. Operator-only, the same bucket as the display mutations above — a camera holds no PIN and makes no GraphQL call of its own. Returns null for a display nobody has seen, or one that is not a `CAMERA`.
-   `setCameraOrder(displayId, order)` (#177 stage 4): tells a `CAMERA`-role display where its clip plays among several — lower plays first, ties (including two untouched cameras, both at the default `0`) broken by `displayId`. Same bucket and refusal shape as `setCameraTrack`. `heatReplay`/`Heat.replays` both sort their clips by this in-memory value (`_order_replay_clips`) rather than the frontend re-sorting them, replacing stage 1's client-only alphabetical order.
-   Display scenes (#613): `createScene(raceId, name)` (snapshots every display currently known for the race — connected or not — into a new named `Scene`), `renameScene(id, name)`, `deleteScene(id)`, `updateSceneDisplay(sceneId, displayId, displayName, view, ...)` (adds or edits one display's entry within a scene; the same argument shape as `assignDisplay`, and an omitted rider keeps what that entry already had), `removeSceneDisplay(sceneId, displayId)`, `applyScene(sceneId)`, `applyScenePreset(raceId, preset)` (both return an `ApplySceneResult` — a `SceneApplyOutcome` per display named, `appliedCount`/`skippedCount` — since a saved scene naming a display that has since gone quiet must not block the rest from being applied; a preset is code, computed live against whichever displays `displays(raceId)` currently returns, and stores nothing). Scenes are stored, unlike a `Display`'s own live assignment — see §3.2.
...
-   Manual advancement (#711): `pinRoundField(raceId, roundId, racerIds)` / `unpinRoundField(raceId, roundId)` — `pinRoundField` rebuilds a championship round for exactly the given racers and sets `Round.fieldPinned`, which the recorded-result cascade and `populate_round_if_decided` both then leave alone rather than re-fielding from the standings; refused for a general round, for a round already raced, and for a pick that is too short, duplicates a racer, or names anyone not checked in (#228). `unpinRoundField` clears the flag and hands an unraced round back to the standings as they stand now. `AdvancementStatus.fieldIsPinned` reports which is in effect, reading the pinned lanes directly rather than the computed pick when set.
...
-   Run-off heats (#550): `createRunOffHeat(raceId, racerIds, settlesRoundId)` / `deleteRunOffHeat(heatId)` — `createRunOffHeat` builds a `RunOffHeat` (`kind = RUN_OFF`, no `roundId`) holding the tied racers, armed and recorded through the ordinary `prepareHeat`/`updateHeatResult` path like any other heat by id. It never feeds an aggregate score, a track record, or the schedule it did not come from — `Race.runOffHeats`, `RunOffHeat.placement` and `Heat.runOffPlacement` are all computed fresh on every read (never stored), and `domain.tiebreak`'s `RUN_OFF` resolution beats the race's own configured tiebreaker whenever a decided run-off exists for exactly the tied cluster in question. `deleteRunOffHeat` removes one that has not been run yet, the operator's undo for a mistake.
-   Intermission (#592): `startIntermission(raceId, durationSeconds, label)`, `extendIntermission(raceId, seconds)`, `pauseIntermission(raceId)`, `resumeIntermission(raceId)`, `endIntermission(raceId)` — all operator-only, all return the `Race`, and all publish `raceStateChanged` with `kind: INTERMISSION` on the race's own `race_state:{raceId}` channel (no new channel) carrying the freshly resolved `Intermission`. `Race.intermission` (`active`, `remainingSeconds`, `paused`, `label`, `endsAt`) is computed on every read from three plain columns (`domain/intermission.py`) — an expired countdown simply resolves inactive, with nothing to sweep. `endsAt` is the one field a client needs to run its own live countdown; `remainingSeconds` is a read-once snapshot. Pre-set, schedule-time intermissions (an issue asked for both) are not built — this is the on-the-fly half only.
-   Config: `createInitialConfig`, `updateInitialConfig` — both accept the organization's terminology default (racing group, organization, and, since #551, vehicle) the same way they accept the PINs: absent leaves it alone, and `clearTerminology` (not a non-null sentinel — the built-in Scouting words *are* the null state) is the way back to it (#496 stage 3). Both also accept the organization's name-display default (#552): `"FULL"` *is* the non-null off state here, so absent leaves it alone and no clear flag is needed at this layer. Neither carries `debugMode` or either theme field any more (#1079, #1080) — see the two mutations below. `updateInitialConfig` alone also accepts `keepReplays`/`replayRetentionHeats`/`replayRetentionMb` (#177 stage 2, absent leaves each alone; `clearReplayRetentionHeats`/`clearReplayRetentionMb` are the way back to unbounded) — deliberately *not* split into a `setDebugMode`-style mutation of its own, since turning stored clips on is a disk write this mutation's own wholesale demo refusal needs to keep blocking
-   `setDebugMode(enabled)` / `setThemes(displayTheme, printablesTheme)` (#1079, #1080) — one writer per field, split out of `updateInitialConfig`'s bundle so the public demo (`demo_policy.py`) can offer these two harmless fields while still refusing the rest of that form, which also sets both PINs and reconfigures tracks. Both operator-only, both return `InitialConfigStatus`, and `setThemes` calls the same `_broadcast_display_theme_change` `updateInitialConfig` already used for `displayTheme`, so an open Observation page re-themes with no reload regardless of which mutation changed the setting
-   Data: `importRacers` (CSV), `previewGprmImport(raceId, fileData)` / `confirmGprmImport(raceId, fileData)` (a GrandPrix Race Manager database, #618 — `fileData` is a base64 data URL like `uploadImage`'s; preview parses and returns a `GprmImportPreview` without writing, confirm re-parses the identical upload and writes it, so nothing is trusted back from the client between the two calls; both `CHECKIN`, matching `importRacers`), `previewDerbynetImport(raceId, fileData)` / `confirmDerbynetImport(raceId, fileData)` (a DerbyNet database, #661 — the identical preview/confirm shape, over the same table family `domain/gprm.py` already reads, returning `DerbynetImportPreview` rather than sharing `GprmImportPreview`; a sibling mutation pair rather than a rename of the GPRM one, so an already-shipped mutation's name did not have to change to add a second importer), `uploadImage` (base64), `populateRace` (test data), `createPracticeRace` (a whole rehearsal event on a fake timer; resumes the most recent one rather than building another unless `startNew` is set, #588)
...
-   `GET /printables/vote-qr/{race_id}.png` and `GET /api/printables/vote-qr/{race_id}.png` — a QR code for the ballot address (#414), encoding the `url` query parameter as-is rather than recomputing it, so the text and the code cannot disagree. `url` is validated (#866): scheme must be `http`/`https`, the path must end with this race's `/vote` or `/observation` address, and the host (and port, if given) must match this server instance. Not cached `immutable`: the address depends on the machine's current network.
...
-   `POST /replay/` (#177 stage 1a) — a camera's replay clip for one heat: `raceId`, `heatId`, `recordedAt`, `cameraId`, `t0OffsetMs`, `durationMs` and the file (`video/webm`/`video/mp4`, capped at 12 MB). No PIN required — the same auth story `displayAssignment`'s subscription already has, since a camera is a display with a role and holds no credential (#15); what actually guards it is `recordedAt` having to equal the heat's *current* `Heat.recordedAt` (409 otherwise — a re-run moves it on, and a stale clip is for a run that no longer exists), plus a 404 if the heat is not on the named race and a 415/413 for the wrong content type or an oversized file. Refused outright on the demo. Written under `DATA_DIR/replays/`, never `uploads/`, with a non-enumerable name — `services/backup.py` never carries one, by construction (it only ever reads `uploads/` by name).
-   `GET /replay/{filename}` (#177 stage 1a) — streams a stored clip, `Cache-Control: no-store`. The filename is a UUID; that is the whole of the access control, the same shape `/static/<filename>` uses for a photograph (#552's reasoning extended to video).
...
-   `subscription timingStats(raceId)` — Per-lane timing for the most recently recorded heat, official or free. Carries `recordBreak` when the heat beat the track record as it stood before this race (`services/records.py`); a free heat never does, since an exhibition run cannot hold a record. Each lane also carries `scaleMph` (#610 stage 4) — that lane's time converted through `domain.scale_speed.scale_mph`, null when the track has scale speed turned off, has no configured length, or the lane has no time.
-   `subscription heats(raceId)` — Full round/heat list with completion status.
-   `subscription displayAssignment(displayId, raceId, name)` — What one screen should show. **Subscribing is how a display registers**: it holds no PIN and is a `VIEWER`, so it can make no mutation, and the socket closing is the only signal it has gone. Also carries `displayThemeSetting` (#586, #1081) — this race's own Display theme override layered over the organization's install-wide one, resolved fresh on every event by `_display_theme_setting` — so a theme change made in System Settings or on the race's own Appearance section reaches an already-open screen over the same socket, with no reload. `setThemes` nudges every connected display's channel (install-wide, #1080); `updateRace` nudges only the displays already pointed at this race (`_publish_race_display_theme_change`, #1081).
-   `subscription displays(raceId)` — The operator's list, as screens come and go.
-   `subscription heatReplay(raceId)` (#177 stage 1a) — The current heat's replay clips, as cameras upload them: `heatId`, `recordedAt` and a list of `ReplayClip` (`cameraId`, `url`, `durationMs`, `t0OffsetMs`). Keyed the same way `resultsOverlay.ts`'s `observeHeatResult` already keys a result — `heatId` plus the exact `recordedAt` the heat carried when the camera captured its buffer — so a display compares this pair against the last one it acted on rather than trusting the subscription's own opening payload, the same `seen === null` rule `identifyOverlay.ts` follows. A snapshot channel: every wake re-reads `ReplayStore.latest_for_race`, so the default bounded, drop-oldest pub/sub queue is safe here (see `CLAUDE.md`'s pubsub paragraph) — unlike `displayAssignment`'s ceremony steps, nothing here is an event a dropped wake-up could lose. Clips are served from `GET /replay/<name>` — not `/static` — and retained per `.claude/rules/displays.md`'s delete-after-next-heat rule (stage 1).
