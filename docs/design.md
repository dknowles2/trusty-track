...
-   Display scenes (#613): `createScene(raceId, name)` (snapshots every display currently known for the race — connected or not — into a new named `Scene`), `renameScene(id, name)`, `deleteScene(id)`, `updateSceneDisplay(sceneId, displayId, displayName, view, ...)` (adds or edits one display's entry within a scene; the same argument shape as `assignDisplay`, and an omitted rider keeps what that entry already had), `removeSceneDisplay(sceneId, displayId)`, `applyScene(sceneId)` and `applyScenePreset(raceId, preset)` (both return an `ApplySceneResult` — a `SceneApplyOutcome` per display named, `appliedCount`/`skippedCount` — since a saved scene naming a display that has since gone quiet must not block the rest from being applied; a preset is code, computed live against whichever displays `displays(raceId)` currently returns, and stores nothing). Scenes are stored, unlike a `Display`'s own live assignment — see §3.2.
