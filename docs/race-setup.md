...
5. (Optional) Set the den's **Category** — a dropdown offers the traditional Cub Scout ranks (Lion, Tiger, Wolf, Bear, Webelos, Arrow of Light) to fill the box, or type anything you like. It is shown beside the den's name in the list and beneath a racer's name on the audience displays, where their den is not otherwise shown. On the Standings page's Den column it is shown beside the den's name too, unless it is just the den's own name again — the setup wizard's ready-made dens start that way, and a den named "Bear" with Category "Bear" prints as plain "Bear", not "Bear (Bear)".
6. Click **Add Den**.
