...
**The boot smoke test itself (layer C) is the one piece still failing, and it is a real, distinct bug** — dknowles2/trusty-track#1235. The kernel boots and the console proves `SERIAL_AMBA_PL011` works, then stalls forever at `Waiting for root device /dev/vda2...`: `kernel8.img` (`raspberrypi/linux`'s `bcm2711_defconfig`) turns out to carry no `CONFIG_VIRTIO_*` support at all, not even as a module, so the virtio-blk root device this script attaches can never appear. The "carries VIRTIO_BLK, VIRTIO_NET ... built in" claim that used to sit here was exactly the same shape of reasoned-but-unconfirmed mistake as the three above — asserted from how the mechanism was supposed to work, corrected only once someone actually ran it. Treat the workflow overall as build-and-layer-B-confirmed, boot-test-still-broken until #1235 lands.
