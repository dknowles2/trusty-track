...
**This session could not run any of it for real** — no privileged Docker, no `qemu-system-aarch64`, no root, in the sandbox stage 2 was written in — so the wiring above is reasoned from pi-gen's and the Raspberry Pi kernel's documented behaviour rather than a green run. There is deliberately no `workflow_dispatch` to test it standalone (`release.yml` triggers on a version tag only, same as always), so the first real exercise is the next tag pushed after this merges; treat that run's log as the thing actually establishing the facts asserted in the two paragraphs above, the same "a performance number belongs to the machine it was measured on" caution `ci.md` already states for CI timings generally — here extended to correctness, not just speed, because nothing about booting a Pi image in QEMU has been measured on any machine yet.
