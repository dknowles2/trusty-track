...
**Stage 2's longer retention (`Organization.keep_replays`, #177) does not change any of the above — a stored clip is exactly as excluded as a stage-1 one.** What it does change is what a *restore* leaves behind in the database: `models.HeatReplay` rows can survive inside the archived `trusty-track.db` even though their files never travel with it, so `POST /api/backup/restore` (`api/main.py`) deletes every `HeatReplay` row outright as part of the restore, right alongside the unconditional `ReplayStore.sweep()` stage 1 already ran there — a surviving row would be a broken link (a Schedule-tab ▶ with nothing behind it) rather than the honest "no stored clips yet" a fresh restore should show.
...
**`rumps.alert()` returns a plain int, not an object** ([#884](https://github.com/dknowles2/trusty-track/issues/884)) — `NSAlert.runModal()`'s own return value, with "ok" as 1 and "cancel" as 0 (rumps' own docstring). `TrustyTrackApp._reset_db` tested `resp.clicked`, which raised `AttributeError` on every click of either button and left the "Reset Database…" menu item silently doing nothing; the Windows sibling right beside it already had the right shape, comparing `MessageBoxW`'s int against `idyes`. `packaging/alert_response.py` holds the comparison as its own tiny pure module — sibling to `run_server.py` for the same reason `http_mode.py`/`log_viewer.py`/`cert_requirements.py` are, and doubly so here: `rumps` itself only imports on macOS (it wraps PyObjC/Cocoa), so a test that needed a real `rumps.alert()` call could only ever run on a Mac, where this bug could not be caught by CI regardless. `backend/tests/test_alert_response.py` exercises the module directly; nothing here runs the real menu item, since that needs a macOS bundle build this repository's tests do not have.
...
`install_node` — the piece of `install_system_packages` that runs the NodeSource step — refuses (`error`, non-zero exit) rather than reporting Node OK when NodeSource's setup script fails to register its apt source or the installed Node still misses the version floor, instead of continuing to `build_app`'s `npm ci` with whatever Debian's own `nodejs` package landed ([#1322](https://github.com/dknowles2/trusty-track/issues/1322) — the v1.5.0 Pi image build hit exactly this under QEMU emulation).
...
**`deploy/raspberry-pi/qemu-boot-smoke-test.sh` is layer C: it boots the actual artefact**, not the chroot layer A checked or the mounted-but-unbooted rootfs layer B checked. It hands the *whole* raw image to QEMU's `raspi3b` machine as an SD card (`-sd`, so the guest sees the same partition table and PARTUUIDs `/etc/fstab` and `cmdline.txt` already name, just a different transport than a real Pi's own SD slot), extracts `kernel8.img` and a matching Broadcom device tree (`bcm2710-rpi-3-b-plus.dtb` or `bcm2710-rpi-3-b.dtb`) from the boot partition to satisfy `-kernel`/`-dtb` (a real Pi's firmware reads `config.txt`/`cmdline.txt` itself; QEMU has none of that, so `-append` supplies the kernel command line directly — reusing cmdline.txt's own `root=`/`rootfstype=`/`fsck.repair=` tokens rather than asserting a device name, since `raspi3b` and `raspi4b` do not agree on which `/dev/mmcblk*` number the SD card enumerates as), and gets a console for free because `SERIAL_AMBA_PL011` is built into `kernel8.img`. Networking is a USB Ethernet adapter (`-device usb-net`) on the guest's DWC2 USB host controller — the same path a real Pi 3B's own onboard LAN7515 chip takes, not a QEMU shortcut. It then polls `/health` and, critically, **checks the version in the answer against the version the image was built as** — the same trick `deploy-demo.yml` already uses against a Cloud Run revision that can otherwise serve traffic while quietly pinned to a stale one. The equivalent mistake here is an image built from the wrong ref, or one where `00-copy-source`'s version stamp silently didn't take; either way the server would still answer *something*; only the version check catches it.

**What layer C does not, and cannot, prove.** QEMU's `raspi3b` models real Broadcom BCM2837 hardware closely enough for this kernel's own SD and USB drivers to bind, but it is still not the Pi 4 this image actually targets, and QEMU's own documentation calls its peripheral model incomplete even for what it does implement (no PWM). Boot firmware, GPIO, real USB serial timers, the touchscreen and actual SD card timing are all untested. The image ships smoke-tested, not hardware-tested — the same footing this project already states plainly for the DerbyNet timer profiles rather than implying support, and every place that describes this image to a reader should say so too.

**The first real run found three bugs a sandbox with no privileged Docker, no `qemu-system-aarch64` and no root could only ever reason about, never see** (dknowles2/trusty-track#1234, three manual `workflow_dispatch` runs on 2026-09-18). None were where the run had ever actually failed before — the host was missing `qemu-user-static`, the exact thing pi-gen's `build-docker.sh` checks for on the host and the thing every earlier attempt (including both original release tags) had died on in the first few seconds. Once that was installed: `stage-trustytrack/00-copy-source`'s rsync excluded `docs/` unanchored, silently dropping `frontend/src/docs/` (real application source) along with the top-level mkdocs site it meant to exclude. Once that was anchored: `pyproject.toml`'s `packages = ["backend"]` turned out to only package the flat files directly inside `backend/`, never any subpackage — a bug that has been true of every *real* (non-editable) `pip install` this project has ever done, including on a live Pi, invisible only because `scripts/install-pi.sh` runs from `WorkingDirectory=/opt/trustytrack`, where the raw source it just rsynced there shadows the venv's own broken copy on `sys.path`. Fixed, the pi-gen build and layer B both go green — the first time either ever has outside a sandbox.

**The boot smoke test's first attempt (layer C) failed, and it was a real, distinct bug, since fixed** — dknowles2/trusty-track#1235. The kernel booted and the console proved `SERIAL_AMBA_PL011` worked, then stalled forever at `Waiting for root device /dev/vda2...`: `kernel8.img` (`raspberrypi/linux`'s `bcm2711_defconfig`) carries no `CONFIG_VIRTIO_*` support at all, not even as a module, so the virtio-blk root device QEMU's generic `virt` machine attached could never appear. The "carries VIRTIO_BLK, VIRTIO_NET ... built in" claim that used to sit here was exactly the same shape of reasoned-but-unconfirmed mistake as the three above — asserted from how the mechanism was supposed to work, corrected only once someone actually ran it. It went deeper than the one device: `virt`'s PCIe bus has no driver in this kernel either (`CONFIG_PCI_HOST_GENERIC` is absent; the only PCIe host bridge driver built in, `CONFIG_PCIE_BRCMSTB`, matches only a real Broadcom SoC's own PCIe controller), so a PCI-attached alternative like `qemu-xhci` would have been a dead end on `virt` too — the bus itself is never probed. `virt` cannot boot this exact kernel by any device short of a non-stock kernel, which this project avoids.

The fix (same issue) switched to `-M raspi3b`, a QEMU machine type that models real Pi 3B hardware this kernel's own SD (`CONFIG_MMC_SDHCI_IPROC`, `CONFIG_MMC_BCM2835_MMC`) and USB (`CONFIG_USB_DWC2`) drivers actually bind to — see the smoke test script's own header for the full reasoning, and RPi-Distro/pi-gen#827 for the prior art it followed. That surfaced one more real-hardware constraint along the way: QEMU's `-sd` enforces a real SD card's own power-of-two capacity requirement, and pi-gen's image (3.02 GiB, sized to its partitions plus slack) is rejected outright rather than rounded, so the script pads a private copy up to the next power of two (`qemu-img resize`, unpartitioned space only — nothing is lost) before booting it. Confirmed on real (if emulated) runs, not reasoned: run [35397698252](https://github.com/dknowles2/trusty-track/actions/runs/35397698252) hit the SD-size rejection first; run [35402725381](https://github.com/dknowles2/trusty-track/actions/runs/35402725381), after the padding fix, booted, mounted root, brought up the USB Ethernet device (past a few benign `usbnet: failed control transaction` retries in the serial log) and answered `/health` with `{"status":"ok","version":"0.0.0-dev-9431e85"}` — the exact version that build was stamped with. Treat the workflow as build-, layer-B- and boot-test-confirmed.
