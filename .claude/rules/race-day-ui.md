...
**The corner toggle is one of several sound effects now (#554), and the two systems are deliberately independent (#871).** `features/audio/soundEffects.ts` holds a `master` flag plus five per-event effects (`stagingReady`, `gateRelease`, `finish`, `recordBreak`, `awardFanfare`), edited from the **Sound options** panel (`SoundSettingsSection.tsx`) that both `RaceExecution` and System Settings embed. The corner checkbox documented above writes only to the legacy `finish` flag/storage key `chime.ts` reads — it does **not** touch `master`, and `master` does not touch it — so `RaceExecution`'s effect plays through the richer per-event path when `master` is on, and falls back to the legacy `chimeOn` flag when it is off. Before [#871](https://github.com/dknowles2/trusty-track/issues/871), checking the corner box (or any single effect inside the panel) silently flipped `master` to `true` too, which meant every *other* effect already left checked in storage — award fanfare, the record-break fanfare — started actually sounding, unasked. Each toggle now only ever writes its own field; don't reintroduce the coupling by making one checkbox's `onChange` reach into another's.
