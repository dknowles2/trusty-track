...
**Entering a PIN reloads the page** — but only a PIN the server has already confirmed (#993). Before this, `UnlockButton.tsx` wrote whatever was typed to `localStorage` and reloaded unconditionally: a wrong guess resolves to `VIEWER`, the same role the device already held, so the reload changed nothing and looked exactly like the button doing nothing — while the wrong PIN now rode along on every later request from that device. `frontend/src/api/verifyPin.ts` asks first, with a **raw `fetch`** to `/api/graphql` carrying the candidate PIN — never the urql client, whose fetch exchange always attaches whatever PIN is *already* stored (`pinHeaders()`) and whose normalized cache is keyed on document-plus-variables, not the header, so running `INITIAL_CONFIG_QUERY` through it with a guess would overwrite the cached `role` for the device's real credential. `initialConfig { role }` is an ordinary query, so `RolePolicyExtension` never enters into it and a guess — always a `VIEWER` until this answers — may ask it freely; nothing about asking writes anything, on a hit or a miss. Only once the answer is not `VIEWER` does `UnlockButton` write the PIN and reload — heavy-handed for four digits, and still deliberate for the reason below: the socket carries the PIN in its URL, so a new PIN needs a new socket, and rebuilding the urql client and its normalized cache mid-session to avoid a reload is a great deal more machinery for something that happens once per device per event.

**A PIN that stops being right on a device that already holds it is not this issue's problem, and is not silently swallowed either.** The operator changing the PIN in Settings only invalidates *that* device's own cached `initialConfig` (`forgetInitialConfig` in `graphqlClient.ts`) — a second device's padlock can still show "unlocked" against a PIN that no longer verifies, because nothing pushes it a fresher answer. But `resolve_role` re-reads the organization's current PIN hashes from the database on every operation, so the next mutation that stale device attempts is refused for real, with the ordinary humane message (`RolePolicyExtension`, "#892" above) — the icon can lag, the enforcement never does.
...
**The `?pin=` on that URL must never reach a log line** ([#745](https://github.com/dknowles2/trusty-track/issues/745)). uvicorn's own connection logging writes the whole request line on every WebSocket accept, refusal and close — and the one that matters is `uvicorn.error`, not `uvicorn.access`: `uvicorn/protocols/websockets/{websockets,wsproto}_impl.py` both log the handshake through `logging.getLogger("uvicorn.error")`, so a fix aimed only at "the access log" by name would miss the line the PIN is actually in. `backend/api/main.py`'s `_PinRedactingFilter` is attached to both loggers at import time (`_install_pin_log_redaction`) and strips `pin=<value>` out of any string a log record carries, on every launcher, with no change to any of them — the same `#48` shape as everything else "honoured everywhere" in this file. Attached to the *logger*, not a handler: uvicorn rebuilds its handlers via `logging.config.dictConfig` (at CLI startup, before this module is even imported), but leaves a logger's own filters alone unless the config names one, which uvicorn's does not — so this survives regardless of which entry point started the server, and in which order. Moving the PIN out of the URL entirely (a `connection_init` payload, say) was considered and set aside: it would need a matching frontend change and would still leave a future log line free to reprint the raw URL, where redaction closes the one path the credential could ever reach a log through, permanently, without touching the auth model at all.
...
**`bulkAssignPhotos` sits beside `uploadImage` in the denylist for the same reason, added late** ([#746](https://github.com/dknowles2/trusty-track/issues/746)). `uploadImage` was refused because it writes a caller-supplied image to disk; `bulkAssignPhotos` writes a caller-supplied *URL* straight onto `Racer.racer_image_url`/`car_image_url` with no upload at all, and those fields are rendered on every public, unauthenticated audience surface (`RacerAvatar`, the projector, printables). `crud.bulk_assign_racer_photos` calls `domain/photos.is_valid_photo_url` itself and skips anything that is not `uploadImage`'s own `/static/...` shape regardless of demo mode, but this entry stays — belt and braces, the same reasoning `uploadImage` gets for the disk. (`RacerBase`/`RacerCreate` carry the same check as a Pydantic validator for a brand-new racer; `RacerUpdate` does not — see `crud.update_racer`'s `_validate_photo_url_change`, [#879](https://github.com/dknowles2/trusty-track/issues/879) — since editing an existing racer has to be able to exempt a value that is not changing.)
...
**`uploadImage` and `importRacers` had no cap of their own** ([#744](https://github.com/dknowles2/trusty-track/issues/744)) — `uploadImage`'s `data_url` is `base64.b64decode`d in `schema.py` with no length check, and `importRacers`'s `csv_data` goes straight into `csv.DictReader`, unlike `MAX_UPLOAD_BYTES` above and the GPRM/DerbyNet importers' own `MAX_GPRM_IMPORT_BYTES`, both of which already measure while reading. `backend/api/main.py`'s `MaxGraphQLBodySizeMiddleware` closes this at the one seam that covers both resolvers (and every other GraphQL mutation) regardless of which argument is carrying the oversized value — the raw HTTP request body to `/graphql`/`/api/graphql`, read off the ASGI `receive` channel and totalled chunk by chunk, refusing with `413` the moment it clears `MAX_GRAPHQL_BODY_BYTES` and never buffering the rest. Sized above the largest body a legitimate request can already produce — a GPRM/DerbyNet import's own `file_data` is about 85 MB on the wire once base64 inflates `MAX_GPRM_IMPORT_BYTES`'s 64 MB decoded cap — so this bounds an unbounded body without tightening an existing one. A per-argument check belongs beside the argument it means something to; this is the boundary-level backstop for the two resolvers that had none, applied without touching `schema.py`.

**`services.image_processing.convert_to_browser_safe_png` corrects EXIF orientation before it resizes or re-encodes, and refuses unreadable bytes with a sentence rather than a stack trace** ([#885](https://github.com/dknowles2/trusty-track/issues/885)). A phone photo is stored in sensor orientation with an `Orientation` tag saying how to display it upright; resizing or format-converting from the raw pixels and re-encoding to PNG (which carries no such tag) bakes the sideways layout in permanently — HEIC always takes this path, since every HEIC upload is converted. The fix is `ImageOps.exif_transpose`, called only on the path that is about to re-encode (needs a resize or a format conversion), not unconditionally: `exif_transpose` always forces a full pixel decode (`image.load()`), which is exactly what the existing "return the original bytes unchanged" fast path exists to avoid paying for on an image that needs no processing at all — and a browser already honours that image's own EXIF tag when serving the untouched bytes, so there is nothing to correct on that path anyway. **`UnreadableImageError`** (a `ValueError` subclass, defined alongside the function) is raised for both `PIL.UnidentifiedImageError` (garbage bytes) and a plain `OSError` (a truncated upload whose header is intact enough for `Image.open` to identify the format, failing only once something reads the pixel data it never received) — both upload doors catch it: `POST /upload/` turns it into a `400`, and `Mutation.upload_image` lets it through as an ordinary `ValueError`, the same shape every other refusal in that resolver already takes. Before this, an uncaught `UnidentifiedImageError` reached neither door's exception handling and surfaced as a raw 500 — the request-body cap above stops an oversized upload, but says nothing about a small, genuinely corrupt one.
...
**`_racer_ids_locked` resolving from `racer_ids[0]` alone was a real hole, closed one layer up, not here** ([#743](https://github.com/dknowles2/trusty-track/issues/743)). A `racerIds` list drawn from two races — one locked, one not — dodged this module's lock check whenever the *first* id happened to belong to the unlocked race, since `_racer_ids_locked` never looked at the rest of the list. The fix is not a change to this file: `api/schema.py`'s `_race_id_for_racers` — the same helper every bulk racer resolver already called to find which race's state to publish — now raises `ValueError` outright when the ids it is given span more than one race, before any of the six bulk resolvers (`bulkAutoNumber`, `bulkClearNumbers`, `bulkCheckIn`, `bulkSetExcludedFromStandings`, `bulkMoveToRacingGroup`, `bulkDeleteRacers`) does anything else. A mixed list is refused whether or not either race is locked, which is what makes `_racer_ids_locked`'s first-id shortcut safe to leave as it is: by the time a resolver's body runs, every remaining id is already known to belong to one race. The five `crud.bulk_*` functions also gained a `race_id` parameter, scoping their own queries to it — belt and braces, for a caller that reaches `crud` directly and skips this check entirely.
