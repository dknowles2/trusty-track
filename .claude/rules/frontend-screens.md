...
**The original/edit pair on `RacerInput` does not follow this input's own "absent means leave alone" convention** — `update_racer`'s resolver pops all four fields and reapplies them unconditionally, ahead of the ordinary `is not None` filter every other field goes through. Unlike `car_name` or `car_weight`, there is no partial update to this pair that makes sense on its own: they move in lockstep with the image itself, `RacerForm` always knows the correct current value (including null) for both, and `RaceDetails.tsx`'s `saveRacer` — the input's one caller — always resends the whole form on every save, the same "full resend" shape every other field on this input already assumes. `clearRacerImage`/`clearCarImage` still override on top, so a payload that (adversarially, or by a future caller's mistake) sent a non-null original alongside a clear flag gets the clear regardless.
