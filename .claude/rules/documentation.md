...
**That residual is now measured, not estimated, and it drives CI's own drift gate** ([#838](https://github.com/dknowles2/trusty-track/issues/838) — see `.claude/rules/ci.md`'s note on the `Doc Screenshots` job). `frontend/e2e/docs/compare-screenshots.ts` runs pixelmatch (`includeAA: false`, its default) between two directories of these images, and across four consecutive clean local runs — nothing changed, just rerun — the "handful of pixels of antialiasing" above measured at exactly **zero** on every image except the ones named here: pixelmatch's own antialiasing model, not a raw byte diff, is what closes the gap between "a dozen images differing by a few dozen pixels" (the old, `diff -rq`-era estimate) and an exact match. That is what makes a threshold tight enough to matter — 50 px / 0.05% — survivable rather than a coin flip: it sits far below the smallest genuine change measured alongside it (a focus ring [#830](https://github.com/dknowles2/trusty-track/pull/830) added to every modal, 1,673–3,980 px depending on the form) and far above the measured-zero noise floor.

**Two images beyond the two above were unstable enough, under repeated measurement, to need an outright exclusion rather than a tolerance** — a tolerance loose enough to absorb their variance would also absorb a change the size of that focus ring, defeating the gate for exactly the class of regression it exists to catch.
...
**Regenerating no longer needs a specific reference machine.** The premise that a development sandbox could not be trusted to regenerate these — because its font rasterisation diverged from whatever produced the checked-in baselines — was the font-selection bug above, not a property of sandboxes in general; `regenerate-doc-screenshots.yml`'s own header is being kept current with this. A local run and CI's `ubuntu-latest` now agree on *which* font renders every glyph, because neither can reach past the bundled families to ask the host. What a local run still cannot promise on its own is a *clean* diff against the checked-in baselines the moment something else has gone stale — the checked-in images accumulate real, unrelated UI changes (a redesigned printable, a new settings control) between runs of the regeneration workflow, same as any screenshot suite nobody runs on every merge, and that backlog shows up in the same diff as a genuine font fix would. Read what actually changed before assuming either cause.
