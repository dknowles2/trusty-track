...
That ruled the first fix out as the cause, so the mechanism was re-derived from the diff images rather than assumed: every failing pixel, on every one of the 40, sat tightly inside the pill's own text glyphs, never on its border or its flat background between them. That is what a background colour caught mid-transition looks like on anti-aliased text — the blended edge pixels shift by a few values, the flat interior mostly does not — and not what a pill fully settled on the wrong side of hover would look like, which is a solid-colour diff across the whole shape. So the pill is not stuck hovered; a transition is genuinely still running when the shutter fires. That is `settleTransitions`'s own failure mode from the paragraph below, not a new one: `Navigation.tsx`'s pill sets its background from `onMouseEnter`/`onMouseLeave` against `transition: all 0.2s ease`, exactly the shape `.settings-nav button`'s CSS `:hover` already needed `settleTransitions` for, and nothing here ever calls it for the pill.

No spec hovers or clicks the pill directly — what triggers it is that Playwright's virtual cursor is never reset between actions or navigations within one spec, so a click on *anything else* earlier in the same flow can leave it resting exactly inside the pill's own centred bounding box once a *later* page happens to render the pill in that same screen position, firing the handler with nobody having asked for it.

Fixed once, in the shared fixture, rather than as a third per-spec call the next element with this shape would otherwise need: `screenshots-setup.ts`'s `page.screenshot` override now moves the pointer off every control (`page.mouse.move(-1, -1)` — off-viewport rather than some "safe" on-page corner, since the pill's own box is page-relative and no fixed on-page point stays safe against a future layout change) and then calls `settleTransitions(page.locator('body'))`, before every screenshot. This is a harness-level fix rather than a `Navigation.tsx` change because the bug is not that the pill's styling is wrong — a hover state that fades in and out is exactly the intended behaviour for a person using the app — it is that a screenshot must never be taken while one is running, the same reasoning `animations: 'disabled'` and `settleTransitions` already rest on everywhere else in this file.
