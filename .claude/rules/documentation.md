...
**#843 was closed once and reopened once — turning the drift gate on for real ([#918](https://github.com/dknowles2/trusty-track/issues/918)) surfaced a second, smaller failure in the same element that the fix above does not reach.** Once the pipefail bug and the rasterisation gap above were both fixed, every one of the 40 images still drifting across two independent CI bootstrap passes differed in exactly one place: the pill again, this time regardless of whether its text was stale, missing, or had never been anything but a static, unchanging string. `getting-started/01-home-page.png` proved that last part — the Home page names no race, so the pill there is always the literal `"Select a Race"`, with no query racing it and nothing for the wait above to catch — and it still drifted.

That ruled the first fix out as the cause, and the diff images pointed at a specific hypothesis: every failing pixel, on every one of the 40, sat tightly inside the pill's own text glyphs, never on its border or its flat background between them — what a background colour caught mid-transition looks like on anti-aliased text (the blended edge pixels shift by a few values, the flat interior mostly does not), not what a pill fully settled on the wrong side of hover would look like (a solid-colour diff across the whole shape). `Navigation.tsx`'s pill sets its background from `onMouseEnter`/`onMouseLeave` against `transition: all 0.2s ease`, the same shape `.settings-nav button`'s CSS `:hover` already needed `settleTransitions` for, and nothing called it for the pill — so `screenshots-setup.ts`'s `page.screenshot` override was changed to move the pointer off-viewport (`page.mouse.move(-1, -1)`, deliberately off-page rather than some "safe" on-page corner, since the pill's own box is page-relative) and then call `settleTransitions(page.locator('body'))`, before every screenshot.

**That hypothesis did not survive two bootstrap passes — the actual mechanism was three more dead ends away yet** ([#938](https://github.com/dknowles2/trusty-track/issues/938), below). Two rounds of push-CI-download-overlay against a branch carrying the fix (each verified against a *fresh* CI run, never a laptop) produced the same ~40-image, pill-only signature both times, with the total pixel count moving from 1,510 to 5,714 between the two passes rather than shrinking toward zero — the opposite of what a genuine fix converging on noise looks like. `settleTransitions` finding nothing to await is consistent with this: no spec ever actually hovers or clicks the pill, so the handler this paragraph describes never fires in the first place, and there is no in-flight transition for a mouse reset to catch. The mouse-move-and-settle change stayed in — it is a harmless, correctly-reasoned defence against the *documented* class of bug, and belongs in the harness rather than `Navigation.tsx` for the same reason `.settings-nav button`'s fix does (a fading hover state is correct product behaviour; a screenshot catching it mid-fade is the harness's problem) — but it is not what is reported below as the fix for #843's residual, because it is not one.

**What the two passes actually show: a small, non-converging pixel difference confined to this one element's anti-aliased text, present even when the text is a static string that never changes** (`getting-started/01-home-page.png`, home page, no race, no query, no hover — "Select a Race" every time). That rules out content races, hover state, and in-flight CSS transitions alike, which is everything the harness has tools for.

**The alpha-compositing hypothesis this raised was tested directly, and it did not hold either.** The pill was the only element on these pages painting anti-aliased text over a translucent layer (`rgba(255,255,255,0.1)`/`0.2`) blended at paint time onto the nav's own opaque background — so `Navigation.tsx`'s background, border and hover-state colours were changed to `color-mix(in srgb, #ffffff <weight>%, var(--scouting-blue))`. Over the nav's own opaque background this is not quite the byte-identical colour a first pass at this reasoning claimed (alpha compositing and `color-mix(in srgb, ...)` are not guaranteed the same operation in every engine), but that turned out not to matter to what was actually tested below, for a reason worth stating plainly: **every comparison that found drift compared `color-mix` against `color-mix`, never `color-mix` against the old `rgba`.** The committed baseline was itself bootstrapped from a clean post-fix run before the failing comparison below ran, and `git diff` between the two commits' `Navigation.tsx` is empty — so a colour difference between the old and new formulas cannot be what the paragraph below measures, whatever that difference turns out to be in the abstract.

Three same-job two-run measurements (`compare-screenshots.ts`'s own threshold-free mode, comparing the regeneration workflow's own two Playwright passes against each other within one job) all found **zero** pill-attributable pixels with `color-mix` in place — one before bootstrapping the baseline, one immediately after, one more against the bootstrapped commit itself (19,169–38,777 total px each time, entirely the already-documented QR-id and clock noise). Each looked like confirmation. None was: bootstrapping a baseline from the first clean run and letting the *real*, sharded `Doc Screenshots` gate compare a subsequent, separately-provisioned CI run against it reproduced the identical signature anyway — 40 of 208 gated images (27 of 104 on one shard, 13 of 104 on the other), 32,632 total drifted pixels, confined to the pill on visual inspection of multiple diffs (including `getting-started/01-home-page.png`'s static "Select a Race" again), with `color-mix` in place on both sides of the comparison, provably. The total is larger than either of the two pre-fix bootstrap passes (1,510 and 5,714), continuing the non-converging pattern rather than breaking it.

**The pattern across all four measurements is the same-job/cross-job split, not fixed-vs-unfixed.** Three separate two-pass comparisons *within a single job* (one Playwright browser instance, one runner, warm caches between the two passes) agree with themselves perfectly on the pill, both before and after this fix. The one comparison *across two independently-provisioned CI jobs* — which is what the real, sharded `Doc Screenshots` gate actually does, and the only shape that has ever caught this residual — disagrees, by more than before. That distinction, not the colour formula, is the leading lead now: whatever is non-deterministic about rendering this element, two Playwright passes sharing a runner process do not exercise it, and two separate runner instances reliably do.

**So the mechanism is not (or not only) blending a translucent layer under the glyphs at paint time** — an opaque, pre-resolved colour drifts by the same shape and a larger magnitude, and it does so specifically (only) when the comparison spans two separately-provisioned runners rather than two passes of one job. The `color-mix` change stayed in (it is not wrong, and removes one candidate mechanism cleanly rather than leaving it half-tested) but is not reported as a fix.

**The mechanism is the hidden version stamp's box, not its ink** ([#938](https://github.com/dknowles2/trusty-track/issues/938)) — and it explains the same-job/cross-job split directly, rather than merely being consistent with it. `HIDE_UNSTABLE` in `screenshots-setup.ts` neutralised `[data-testid="app-version"]` with `visibility: hidden` alone, which hides the ink but keeps the box: the element still occupies the rendered width of `v{version}`, in a proportional font, and that string is built from the git hash — so its width changes on every commit even though nothing about it is ever visible. That element sat in `Navigation.tsx`'s right-hand `flexShrink: 0` group, immediately beside the `flex: 1` container the race-selector pill centres itself in, so the pill — the one element on every page whose position is not anchored — shifted by half of whatever the hash's rendered width changed by. (It no longer sits there at all: [#946](https://github.com/dknowles2/trusty-track/issues/946) moved the version stamp and the GitHub link out of the header entirely, onto the System Settings footer and the mobile drawer's own footer. `HIDE_UNSTABLE`'s width-collapsing half stopped being load-bearing for pill drift at that point, but stayed, since it costs nothing on an element with no sibling left to displace.) **Two Playwright passes inside one job check out the identical commit and so hide the identical width; the real gate compares against a baseline committed on a different commit, and bootstrapping that baseline is itself a commit** — which is exactly why every bootstrap attempt invalidated the very thing it had just measured clean, and why the pixel count *grew* pass over pass (1,510 → 5,714 → 32,632) rather than converging: each pass moved the pill again before the comparison ran. Neither the hover-transition fix nor `color-mix` was the cause — both were reasonable, both removed a real candidate mechanism, and neither could have closed this one, because neither touches layout. The fix is `width: 0` plus `overflow: hidden` alongside the existing `visibility: hidden` in `HIDE_UNSTABLE`, so the element's box can no longer vary with the string it is hiding — see `.claude/rules/ci.md`'s note on the same run numbers for the bootstrap that followed.
...
- **A CSS transition can lose to its own screenshot.** `.settings-nav button`'s `transition: background-color 0.2s` (the plain `button` rule in `index.css`) is what animates the active pill when a sectioned form's nav changes — `race-setup/11-edit-race-settings.png` sometimes caught "Event" and "Scoring" both mid-fade, back when that picture was taken by opening the dialog on Event and clicking into Scoring. `page.screenshot({ animations: 'disabled' })` only finishes an animation that already exists in `document.getAnimations()` at the instant it is asked; a transition triggered by the click just before does not always exist yet, because the browser has not necessarily run the style recalc that instantiates it. `settleTransitions(locator)` in `screenshots-setup.ts` waits two `requestAnimationFrame` ticks (giving that recalc somewhere to happen) and then awaits every animation's own `finished` promise directly, rather than guessing how long `0.2s` needs in wall time the way a `waitForTimeout` would. `settleTransitions` is still the right tool for a transition genuinely triggered mid-spec elsewhere; it stopped being what protects *this* image once [#970](https://github.com/dknowles2/trusty-track/issues/970) removed the click that triggered the transition in the first place — the picture now opens straight onto Scoring (`RaceForm`'s `initialSection` prop), so there is no nav transition here for it to catch mid-fade any more.
- **`Modal.tsx`'s dialog element is itself the scrollable container**, and nothing ever resets its `scrollTop`. An action that has to scroll something into reach — filling a field near the foot of a tall form, clicking a control the previous section's shorter layout had left out of view — leaves the dialog scrolled by whatever that took, and the next screenshot shows it exactly there: `race-day/02-check-in-modal-inspected.png` sometimes lost the "Racer Check In" heading entirely. The `page.screenshot` override in `screenshots-setup.ts` now resets every open `[role="dialog"]` to `scrollTop = 0` before every page-level screenshot; a `dialog.screenshot()` or other locator-scoped capture does not go through that hook and has to do its own. `screenshot-race-settings.spec.ts` was that case, back when its own click into Scoring was what needed the reset — [#970](https://github.com/dknowles2/trusty-track/issues/970) removed the click along with the reset it had needed, since the picture now opens straight onto Scoring with nothing to scroll away from the top in the first place.
- **Recharts drives a `<Bar>`'s entrance with its own `requestAnimationFrame` loop, invisible to `animations: 'disabled'`**, which only reaches CSS transitions and Web Animations. `race-stats/01-stats-tab-nav.png`, taken shortly after the page (and its charts) mount, sometimes caught a bar mid-grow. Recharts' own `<Bar isAnimationActive="auto">` — the default, and what `RaceStats.tsx` uses — already checks `prefers-reduced-motion` and skips the animation when it is set, so `page.emulateMedia({ reducedMotion: 'reduce' })` once in the shared fixture fixes it with no production code change.
...
`settings/04-activity-log.png` (real clock times) and `printables/check-in-scanner.png` (where the camera viewfinder sits) were the first two images found to differ run to run, back when "differ" meant a `diff -rq`, and for a while stayed the only two with a known cause. That raw-byte comparison made the residual look far worse than it was: **a high-contrast, high-frequency pattern turns a one-pixel rounding difference into a large-looking byte diff** — a QR code's modules or a portrait's circular crop edge have no antialiasing gradient to absorb a sub-pixel shift into, so `printables/check-in-code-card.png` and `pit-pass-card.png` could report a "255" max delta over a couple thousand pixels for a change invisible to the eye — which is what produced the old "about a dozen images" estimate this paragraph used to give. Diffing two runs' crops directly (before pixelmatch existed to do it automatically, below) is what showed that residual was rasterisation only, not *selection*: #821 closed the much larger failure where the picture showed a different **font** depending on the machine or the moment the shutter fired — see the font wait above. An OS or browser update can still re-rasterise the navigation bar's race pill in *every* image at once; that is a one-time step to commit and move past, not a regression. A run that rewrites fifty images with **content** changes (different racers in lanes, different times) means something above has been undone.

**That residual was measured, not estimated, from the day the tool existed — but it took until #918 for CI to actually check anybody's arithmetic** ([#838](https://github.com/dknowles2/trusty-track/issues/838) — see `.claude/rules/ci.md`'s note on the `Doc Screenshots` job). `frontend/e2e/docs/compare-screenshots.ts` runs pixelmatch (`includeAA: false`, its default) between two directories of these images, and across four consecutive clean local runs — nothing changed, just rerun — the "handful of pixels of antialiasing" above measured at exactly **zero** on every image except the ones named here: pixelmatch's own antialiasing model, not a raw byte diff, is what closes the gap between "a dozen images differing by a few dozen pixels" (the old, `diff -rq`-era estimate) and an exact match. That local measurement stands on its own. **What did not hold is the belief that CI's own gate had corroborated it**: a `bash -e` default with no `pipefail` swallowed the comparison's exit code behind `| tee -a "$GITHUB_STEP_SUMMARY"` from the step's first run, so "Doc Screenshots" going green after #840 proved nothing about pixels — it could not have failed had the residual been enormous. 50 px / 0.05% is still the right threshold — it sits far below the smallest genuine change measured alongside it (a focus ring [#830](https://github.com/dknowles2/trusty-track/pull/830) added to every modal, 1,673–3,980 px depending on the form) — but the two images below are the proof the check was never live: they were declared "confirmed invisible to pixelmatch" in #840 and, the first time #918 made the gate able to fail, promptly failed it.

**Six images beyond the two above were unstable enough, under repeated measurement, to need an outright exclusion rather than a tolerance** — a tolerance loose enough to absorb their variance would also absorb a change the size of that focus ring, defeating the gate for exactly the class of regression it exists to catch.

- **`observation/08-displays-panel.png` and `observation/13-qrcode.png` depend on state nothing here seeds.** `screenshot-observation.spec.ts` opens fresh browser contexts for its extra display tabs, and each mints its own `displayId` (`displayIdentity.ts`) — unlike the fake timer's lane times or `populateRace`'s invented roster, this is not something `TRUSTYTRACK_DEMO_SEED` reaches, so the whimsical name drawn for whichever tab is not explicitly renamed varies run to run. The vote QR code's payload depends similarly on this machine's own detected LAN address (`services/network.lan_addresses()`, "best-effort" by its own docs), which is why `regenerate-doc-screenshots.yml`'s own PR template already anticipated `08-displays-panel.png` showing up unprompted (#723's address block) before this issue gave the reason a name.
...
**A fifth image and three siblings sat here too, and are fixed rather than excluded** ([#841](https://github.com/dknowles2/trusty-track/issues/841)). The four `settings/*.png` images (`01-lanes-in-service`, `05-track-records`, `06-fake-timer-selected`, `07-no-timer-selected`) traced to a stale assumption in `screenshot-timers.spec.ts`'s header comment, which claimed the file "sorts after screenshot-settings" on the strength of alphabetical file order. That was never a guarantee this config gives: `fullyParallel: false` orders the tests *inside* one file, not the files against each other, and this config's `screenshots` project runs every file in parallel across workers — so `screenshot-timers.spec.ts`'s own temporary proxy-mode track (and, it turned out, any of the other tracks that `screenshot-race-stats`, `screenshot-observation`, `screenshot-free-race` and `screenshot-manual-entry` bring and never delete) could still exist, on a different worker, at the moment `screenshot-settings.spec.ts` took its locator-scoped screenshots of its *own* track card. Locator-scoping alone was not enough: a sibling card appearing or disappearing changes the page's total height, which changes the fractional scroll position the target card gets cropped at, nudging the whole capture by a pixel that has nothing to do with what it pictures — visible as a near-uniform shift through a card's own text, not as another track's content leaking in. A `dependencies` edge forcing the timers spec into its own project, strictly after `screenshots`, was tried and reverted: Playwright pulls a shard containing that project's one test's entire dependency chain into the *same* shard, which under `--shard=1/2`/`--shard=2/2` left one shard running everything and the other running nothing — defeating the two-way split `ci.yml` relies on. The fix that survives sharding lives in the specs themselves: both `screenshot-settings.spec.ts` and `screenshot-timers.spec.ts` now hide every other `track-card-*` element before screenshotting their own, via `page.evaluate`, so the layout each card renders into no longer depends on how many other tracks exist on the shared backend at that instant. Measured stable across a dozen runs, mixing paired (`screenshot-settings` + `screenshot-timers` only) and full-suite runs, before the exclusion was removed from `ci.yml`.

**Screenshots are generated where they are checked. This section previously claimed a local run and CI would agree, and that claim was wrong.** [#931](https://github.com/dknowles2/trusty-track/issues/931) found what re-regenerating locally, over and over, could not converge on for [#925](https://github.com/dknowles2/trusty-track/pull/925): [#821](https://github.com/dknowles2/trusty-track/issues/821)/[#825](https://github.com/dknowles2/trusty-track/pull/825) fixed font *selection* — which family renders, confirmed by layout matching to the pixel across hosts — and that holds. They did not, and could not have, fixed font *rasterisation* — how FreeType turns a chosen glyph into pixels, which depends on the library version and its hinting/subpixel configuration, not on which font file was picked. Three things established the gap, all from turning the drift gate on for the first time. The two CI shards' failing sets were almost disjoint — 47 of 104 and 41 of 104, overlapping on only 4, union 84 — which is what it looks like when a shard that only ever regenerates its own slice (see the sharding note above) finds *every* image it touches differs from what is committed; it is not "84 of 208 images drifted", it is essentially every image CI regenerates. The diffs highlighted every glyph of every line while line breaks and box geometry stayed pixel-aligned — a different font moves layout, stale content is localised, so a uniform per-glyph difference at identical metrics is rasterisation and nothing else. And the identical commit, regenerated in a development sandbox, compared clean at 95 of 104 against the same committed baseline — self-consistent, and systematically different from what CI produces from the same source. **A baseline generated outside CI can therefore never pass a gate enforced inside it**, however carefully or how many times it is regenerated. The fix is not a more careful local regeneration; it is to stop treating any machine but CI's own runner as a source of baselines. `ci.yml`'s `doc-screenshots` job now uploads exactly the images each shard regenerated (`git status --porcelain` against the working tree, taken right after the specs run, names them unambiguously), and `regenerate-doc-screenshots.yml` runs on the same `ubuntu-latest` image — both, not a laptop, are where a baseline is allowed to come from. Three Chromium launch flags (`--font-render-hinting=none`, `--disable-font-subpixel-positioning`, `--disable-lcd-text`, set in `playwright.screenshots.config.ts`) narrow the class of host-dependent rasteriser input the two disagree on; they are not a guarantee that any two FreeType versions agree on an unhinted glyph, so "generated where checked" stays the rule rather than something the flags are trusted to make unnecessary.
