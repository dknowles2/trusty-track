...
About a dozen images still differ run to run, and `settings/04-activity-log.png` (real clock times) and `printables/check-in-scanner.png` (where the camera viewfinder sits) are the two with a known cause. A handful more can differ by a few dozen pixels of sub-pixel antialiasing — the same font file can still hint or antialias a glyph a shade differently between two machines, and after an OS or browser update, the navigation bar's race pill can re-rasterise in *every* image at once, a one-time step to commit and move past, not a regression. **A high-contrast, high-frequency pattern turns a one-pixel rounding difference into a large-looking byte diff** — a QR code's modules or a portrait's circular crop edge have no antialiasing gradient to absorb a sub-pixel shift into, so `printables/check-in-code-card.png` and `pit-pass-card.png` can report a "255" max delta over a couple thousand pixels for a change invisible to the eye; diff two runs' crops directly before assuming a large number means a real one. That residual is rasterisation only, not *selection*: #821 closed the much larger failure where the picture showed a different **font** depending on the machine or the moment the shutter fired — see the font wait above. A run that rewrites fifty images with **content** changes (different racers in lanes, different times) means something above has been undone.
