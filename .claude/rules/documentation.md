...
**Regenerating no longer needs a specific reference machine — on the strength of the local measurement, not a CI one.** The premise that a development sandbox could not be trusted to regenerate these — because its font rasterisation diverged from whatever produced the checked-in baselines — was the font-selection bug above, not a property of sandboxes in general; `regenerate-doc-screenshots.yml`'s own header is being kept current with this. A local run and CI's `ubuntu-latest` should agree on *which* font renders every glyph, because neither can reach past the bundled families to ask the host — that is the reasoning, and it is sound, but it was never actually exercised as a cross-environment check: this file used to cite `Doc Screenshots` passing in CI as confirmation that a Debian sandbox's screenshots matched a real `ubuntu-latest` runner's, which is exactly the claim #918 found could not have been true either way — the gate that would have disconfirmed a divergence could not fail, on any runner, until #918 fixed it. The font-determinism reasoning stands on the local two-run measurements above; CI's agreement is now something the fixed gate can actually attest to going forward, not something it already had. What a local run still cannot promise on its own is a *clean* diff against the checked-in baselines the moment something else has gone stale — the checked-in images accumulate real, unrelated UI changes (a redesigned printable, a new settings control) between runs of the regeneration workflow, same as any screenshot suite nobody runs on every merge, and that backlog shows up in the same diff as a genuine font fix would. Read what actually changed before assuming either cause.
