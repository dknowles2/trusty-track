...
**What the two passes actually show: a small, non-converging pixel difference confined to this one element's anti-aliased text, present even when the text is a static string that never changes** (`getting-started/01-home-page.png`, home page, no race, no query, no hover — "Select a Race" every time). That rules out content races, hover state, and in-flight CSS transitions alike, which is everything the harness has tools for. The leading unconfirmed hypothesis is that this is a genuine run-to-run Chromium rendering non-determinism specific to this element — bold white text anti-aliased against a semi-transparent background that is itself alpha-blended over the nav's own colour, a compositing path nothing else on these pages exercises the same way — rather than anything a spec is doing wrong. `Doc Screenshots` is still red on this residual; see `.claude/rules/ci.md`'s note on the same run numbers. This is an open problem, not a closed one — the next step is confirming or ruling out the compositing hypothesis, not another bootstrap pass.
