...
- **A CSS transition can lose to its own screenshot.** `.settings-nav button`'s `transition: background-color 0.2s` (the plain `button` rule in `index.css`) is what animates the active pill when a sectioned form's nav changes — `race-setup/11-edit-race-settings.png` sometimes caught "Event" and "Scoring" both mid-fade. `page.screenshot({ animations: 'disabled' })` only finishes an animation that already exists in `document.getAnimations()` at the instant it is asked; a transition triggered by the click just before does not always exist yet, because the browser has not necessarily run the style recalc that instantiates it. `settleTransitions(locator)` in `screenshots-setup.ts` waits two `requestAnimationFrame` ticks (giving that recalc somewhere to happen) and then awaits every animation's own `finished` promise directly, rather than guessing how long `0.2s` needs in wall time the way a `waitForTimeout` would.
