...
- **`instant-replay/04-replay-playback.png`, `11-intermission-highlights.png` and `12-finish-frame.png` photograph a `<video>` decoded from a WebCodecs clip, and the clip's own content — not merely which frame of it happens to be on screen — is what varies** ([#1204](https://github.com/dknowles2/trusty-track/issues/1204)). The obvious first fix, and the one tried first: pause the video and seek to a fixed, in-bounds point before the screenshot, rather than photographing it 300ms after it becomes visible while still autoplaying (`screenshot-instant-replay.spec.ts`'s `pauseAndSeekToFixedFrame`, and for 12 — already paused by the finish-mark click — waiting for the seek's own `seeked` event rather than a fixed sleep). That fix is still in the spec, because it is a genuine improvement and the right thing to do regardless: two runs of the *unfixed* spec measured 04 at 2173–28503 px (0.24–3.09%) and 11 at 2097–28774 px (2.40–3.12%); fixed, 04 still ranged 2163–26795 px (0.23–2.91%) and 11 dropped to 0–664 px (0–0.07%) — better, and sometimes exactly zero, but not reliably under the 50px gate. Diffing the images themselves (`pixelmatch`'s own diff output) shows why: the drift is confined to the moving gradient bar in `fake-camera.webm`'s test pattern (`ffmpeg`'s `testsrc` filter draws it as a deterministic function of *frame index*, not wall time), and `capture.ts`'s `fakeCameraTrack` draws that pattern from a real, continuously-looping `<video>` onto a canvas — so which frame index lands at a given point in the *cut* clip depends on how much real wall-clock time had elapsed since the loop started when the capture window began, which varies run to run for the same reason every other capture-timing race in this file does. Seeking inside the clip afterwards cannot fix this: the clip's own bytes already differ before the spec ever touches them. `12-finish-frame.png`'s own residual (133–281 px, down from 221–491 before the fix) is a second, smaller cause — the native video-controls timestamp text (`0:05 / 0:0X`), which reads the clip's own decoded duration and so wobbles by a frame along with it. Fixing either at the source means making `fakeCameraTrack`'s test pattern non-animated or its capture start wall-clock-independent — a change to the shared capture fixture `functional/instantReplay.spec.ts` also exercises, out of proportion to a docs-only fix — so these three are excluded rather than stabilised, the `maxDiffPixels`/`maxDiffRatio`-eluding case the drift-policy file's own comment already names: "excluded here only when it differs between two runs of an unchanged tree for a reason no regeneration can settle."
