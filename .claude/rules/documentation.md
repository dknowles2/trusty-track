...
**So the mechanism is not (or not only) blending a translucent layer under the glyphs at paint time** — an opaque, pre-resolved colour drifts by the same shape and a larger magnitude, and it does so specifically (only) when the comparison spans two separately-provisioned runners rather than two passes of one job. The `color-mix` change stayed in (it is not wrong, and removes one candidate mechanism cleanly rather than leaving it half-tested) but is not reported as a fix. `Doc Screenshots` is still red on this residual; see `.claude/rules/ci.md`'s note on the same run numbers. **This is an open problem, now narrowed rather than merely open**: the next thing worth checking is what genuinely differs between two Playwright passes in one job and two independently-provisioned `ubuntu-latest` runners — font cache state, GPU/software rasteriser initialisation, or simply heterogeneous hardware across the runner fleet — before reaching for Chromium's compositing flags (`--disable-gpu-compositing`, `--disable-partial-raster`, `--force-color-profile=srgb`), which remain the next lever but are no longer the only one worth naming.
