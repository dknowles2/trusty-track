...
**Six images beyond the two above were unstable enough, under repeated measurement, to need an outright exclusion rather than a tolerance** — a tolerance loose enough to absorb their variance would also absorb a change the size of that focus ring, defeating the gate for exactly the class of regression it exists to catch. Two causes, both distinct from rasterisation:
...
- **The four `settings/*.png` images (`01-lanes-in-service`, `05-track-records`, `06-fake-timer-selected`, `07-no-timer-selected`) trace to a stale assumption in `screenshot-timers.spec.ts`.** Its own header comment claimed the file "sorts after screenshot-settings" — true only for a single worker. `fullyParallel: false` orders the tests *inside* one file, not the files against each other, and this config's `screenshots` project runs every other file in parallel across workers — so `screenshot-timers.spec.ts`'s own temporary proxy-mode track can still exist, on a different worker, at the moment `screenshot-settings.spec.ts` takes its locator-scoped screenshots of its *own* track card. A `dependencies` edge forcing the timers spec into its own project, strictly after `screenshots`, was tried and reverted: Playwright pulls a shard containing that project's one test's entire dependency chain into the *same* shard, which under `--shard=1/2`/`--shard=2/2` left one shard running everything and the other running nothing — defeating the two-way split `ci.yml` relies on. Left as an explicit exclusion; a fix that does not cost the shard balance is open.
