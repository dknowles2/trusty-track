...
def _setup_race_with_heats(db, num_racers=3):
    ...
    track = crud.create_track(db, schemas.TrackCreate(name=f"Regen Track {suffix}", lane_count=4))
    race = crud.create_race(db, schemas.RaceCreate(name=f"Regen Race {suffix}", group_id=group.id, track_id=track.id))

    racers = []
    for i in range(num_racers):
        r = crud.create_racer(db, schemas.RacerCreate(
    ...
    round_unstarted = crud.create_round(db, race.id, 1, models.SchedulingStrategy.PPC, "Unstarted Round")
    crud.generate_heats_for_round(db, round_unstarted.id)
    
    # Create regular round (started)
    round_started = crud.create_round(db, race.id, 2, models.SchedulingStrategy.PPC, "Started Round")
    crud.generate_heats_for_round(db, round_started.id)
    heat_started = db.query(models.Heat).filter(models.Heat.round_id == round_started.id).first()
    # Record a result for the first heat
    results_json = json.dumps([{"lane": 1, "racer_id": racers[0].id, "time": 3.0, "place": 1}])
    crud.record_heat_result(db, heat_started.id, results_json)
    
    # Create free race heat
    free_heat = crud.create_free_race_heat(db, race.id, [{"lane": 1, "racer_id": racers[0].id}, {"lane": 2, "racer_id": racers[1].id}])
...
def test_delete_racer_regenerates_unstarted_round(db):
    ...
    race_id, racers, unstarted_id, started_id, free_id = _setup_race_with_heats(db, num_racers=3)
    r1_id = racers[0].id
    
    # Initial check
    initial_heats = db.query(models.Heat).filter(models.Heat.round_id == unstarted_id).all()
    ...
    heats_unstarted = db.query(models.Heat).filter(models.Heat.round_id == unstarted_id).all()
...
def test_delete_racer_nullifies_started_round(db):
    ...
    # Initial check
    initial_heats = db.query(models.Heat).filter(models.Heat.round_id == started_id).all()
    assert len(initial_heats) == 3
    ...
    heats_started = db.query(models.Heat).filter(models.Heat.round_id == started_id).all()
...
def test_delete_racer_nullifies_free_heat(db):
    ...
    free_heat = db.query(models.FreeRaceHeat).filter(models.FreeRaceHeat.id == free_id).first()
    assignments = json.loads(free_heat.lane_assignments)
    assert assignments[0]["racer_id"] is None, "Deleted racer should be nullified in free heat"
