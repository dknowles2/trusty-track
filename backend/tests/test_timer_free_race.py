...
async def test_free_race_heat_recording_in_manager(db):
    ...
    with (
        patch(
            "backend.api.schema._publish_race_state", new_callable=AsyncMock
        ) as mock_publish,
        patch("backend.services.timer.manager.SessionLocal", return_value=mock_session),
    ):
...
async def test_official_heat_recording_in_manager(db):
    ...
    r1 = crud.create_racer(
        db, schemas.RacerCreate(first_name="Bob", last_name="Official", race_id=race.id)
    )
