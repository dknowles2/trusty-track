...
def test_upload_image_mutation(client):
    ...
    # A genuine 1x1 PNG, generated by Pillow rather than hand-typed (#911:
    # the identical hand-assembled bytes in test_upload_image.py's
    # `_TINY_PNG` had a corrupt IDAT stream that only `Image.open(...).load()`
    # ever caught — `Image.open` alone parses just the header).
    png_bytes = (
    ...
    # Verify the file was actually saved, and is a genuinely decodable image
    # rather than bytes that merely start with a PNG header.
    from backend.db.database import UPLOAD_DIR

    filename = url.split("/static/")[1]
    file_path = os.path.join(UPLOAD_DIR, filename)
    assert os.path.exists(file_path)
    with Image.open(file_path) as written:
        written.load()
        assert written.size == (1, 1)
