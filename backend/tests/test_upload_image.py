...
# A genuine 1x1 PNG for valid-image tests — generated by Pillow itself
# (`Image.new("RGB", (1, 1)).save(buf, format="PNG")`), not hand-assembled.
#
# #911: an earlier, hand-typed version of this constant had a corrupt IDAT
# stream — `Image.open` would parse the header (enough to report a size) but
# `.load()` raised `OSError: broken data stream`. It passed every test here
# for as long as it did only because `uploadImage`'s fast path
# (`convert_to_browser_safe_png`'s "already browser-safe, return unchanged"
# branch) never forces a full pixel decode of an RGB PNG within size — so
# nothing in the mutation itself ever proved this fixture decodable, and
# nothing in the tests did either. `test_upload_image_within_limit_succeeds`
# below now forces `.load()` on the file this mutation actually wrote, so a
# future fixture this shape is wrong would fail there rather than pass
# silently.
_TINY_PNG: bytes = (
...
def test_upload_image_within_limit_succeeds() -> None:
    ...
    #911: the fast "already browser-safe" path in `uploadImage` never forces
    a full pixel decode of an RGB PNG, so a write succeeding proves nothing
    about whether the bytes on disk are actually a valid image — a
    corrupted fixture passed this test for a long time. Loading the written
    file here is what a fixture regression would actually fail on.
    """
    mutation = schema.Mutation()
    ...
    # Prove the file is a genuinely decodable image, not just bytes that
    # happen to have a PNG header — `.load()` forces the full pixel decode
    # that `Image.open` alone does not.
    with Image.open(saved_path) as written:
        written.load()
