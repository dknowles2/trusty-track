...
def test_race(db):
    ...
    for i in range(3):
        heat = models.Heat(
            race_id=race.id, round_id=round_obj.id, heat_number=i + 1, lane_results=None
        )
        db.add(heat)
        heats.append(heat)
    db.commit()
    ...
    return {"race": race, "round": round_obj, "heats": heats, "racers": racers}
...
def test_reorder_heats_different_rounds_error(db, test_race):
    ...
    heat_round2 = models.Heat(
        race_id=race.id, round_id=round2.id, heat_number=1, lane_results=None
    )
    db.add(heat_round2)
    db.commit()
