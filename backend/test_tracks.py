...
def test_track_crud():
    ...
    init_data = {
        ...
        "tracks": [
            {"name": "Track A", "lane_count": 4, "length_feet": 40, "timer_type": "FAKE"},
            {"name": "Track B", "lane_count": 6, "length_feet": 50, "timer_type": "FAKE"}
        ]
    }
    ...
    # 2. Add another track
    resp = client.post("/tracks/", json={"name": "Track C", "lane_count": 8, "timer_type": "FAKE"})
    assert resp.status_code == 200
    track_c = resp.json()
    ...
    # 4. Update track
    resp = client.put(f"/tracks/{track_c_id}", json={"name": "Updated Track C", "lane_count": 2})
    assert resp.status_code == 200
    ...
    race_a_resp = client.post("/races/", json={"name": get_unique_name("Race A"), "group_id": group_id, "track_id": track_a_id})
    assert race_a_resp.status_code == 200
    race_a = race_a_resp.json()
    assert race_a["track_id"] == track_a_id
    
    race_b_resp = client.post("/races/", json={"name": get_unique_name("Race B"), "group_id": group_id, "track_id": track_b_id})
...
def test_race_track_association_update():
    ...
    init_data = {
        "group_name": group_name,
        "tracks": [
            {"name": "T1", "lane_count": 4},
            {"name": "T2", "lane_count": 6}
        ]
    }
    ...
    race_resp = client.post("/races/", json={"name": get_unique_name("Update Race"), "group_id": group_id, "track_id": t1_id})
    race = race_resp.json()
    race_id = race["id"]
