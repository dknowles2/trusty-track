...
def test_generate_schedule_only_checked_in(client, db):
    ...
    r1 = schemas.RacerCreate(
        first_name="Checked", last_name="In", car_number=1, race_id=race_id,
    ...
    r2 = schemas.RacerCreate(
        first_name="Not", last_name="CheckedIn", car_number=2, race_id=race_id,
    ...
    r3 = schemas.RacerCreate(
        first_name="Pending", last_name="Inspection", car_number=3, race_id=race_id,
        car_passed_inspection=False
    )
    ...
    if "errors" in resp.json():
        ...
        if len(all_racer_ids) > 1:
            pytest.fail(f"Bug reproduced: Scheduled {len(all_racer_ids)} racers, but only 1 was checked in.")
...
def test_generate_schedule_multiple_checked_in(client, db):
    ...
    for i in range(2):
        r = schemas.RacerCreate(
            first_name=f"Checked{i}", last_name="In", car_number=10+i, race_id=race_id,
            car_passed_inspection=True
        )
    ...
    for i in range(2):
        r = schemas.RacerCreate(
            first_name=f"NotChecked{i}", last_name="In", car_number=20+i, race_id=race_id,
            car_passed_inspection=False
        )
        crud.create_racer(db, r)
    ...
    assert len(all_racer_ids) == 2, f"Expected 2 racers scheduled, but got {len(all_racer_ids)}"
