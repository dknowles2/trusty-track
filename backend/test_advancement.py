...
def test_full_advancement_flow():
    ...
    race_resp = client.post("/races/", json={
    ...
    den_resp = client.post(f"/races/{race_id}/dens", json={"name": "Bears", "color": "#ff0000"})
    ...
    for i in range(4):
        resp = client.post("/racers/", json={
            "first_name": f"Racer", "last_name": str(i),
            "car_number": 100 + i, "den_id": den_id, "race_id": race_id
        })
    ...
    wizard_config = {
        ...
        "championship_rounds": [{
            "name": "Finals", "source": "PACK", "num_top_racers": 2, "runs_per_lane": 1
        }]
    }
    ...
    for h in heats_r1:
        ...
        for res in lane_res:
            rid = res["racer_id"]
            if rid == racer_ids[0]: res["time"] = 1.0 # RID 1
            elif rid == racer_ids[1]: res["time"] = 2.0 # RID 2
            elif rid == racer_ids[2]: res["time"] = 3.0 # RID 3
            else: res["time"] = 4.0 # RID 4
            res["place"] = 1
        client.put(f"/heats/{h['id']}", json={
            "race_id": race_id,
            "round_id": round1_id,
