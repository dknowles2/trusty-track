...
def read_dens(
    race_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
...
def create_den(race_id: int, den: schemas.DenCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="Den with this name already exists in this race"
        )
...
def update_den(
    den_id: int, den_update: schemas.DenUpdate, db: Session = Depends(get_db)
):
...
def update_race(
    race_id: int, race_update: schemas.RaceUpdate, db: Session = Depends(get_db)
):
...
def create_initial_config(
    config: schemas.InitialConfigCreate, db: Session = Depends(get_db)
):
    ...
    if tracks:
        db.query(models.Race).filter(models.Race.track_id == None).update(
            {models.Race.track_id: tracks[0].id}
        )
        db.commit()

    return schemas.InitialConfigStatus(
        initialized=True, group_name=group.name, tracks=tracks
    )
...
def update_initial_config(
    ...
    if group and group.name != config.group_name:
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail=f"Group '{config.group_name}' already exists"
            )
    ...
    tracks = crud.get_tracks(db)
    return schemas.InitialConfigStatus(
        initialized=True, group_name=group.name if group else None, tracks=tracks
    )
...
def update_track(
    track_id: int, track_update: schemas.TrackBase, db: Session = Depends(get_db)
):
    db_track = crud.get_track(db, track_id)
    if not db_track:
        raise HTTPException(status_code=404, detail="Track not found")
...
def update_racer(
    racer_id: int, racer_update: schemas.RacerUpdate, db: Session = Depends(get_db)
):
...
def populate_race(
    race_id: int,
    request: schemas.PopulateTestDataRequest,
    db: Session = Depends(get_db),
):
...
async def import_racers_csv(
    race_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)
):
    ...
    for row_idx, row in enumerate(csv_reader):
        try:
            ...
            car_number = (
                int(car_number_str)
                if car_number_str and car_number_str.isdigit()
                else None
            )
    ...
    return {"message": f"Successfully imported {added_count} racers.", "errors": errors}


@app.post("/races/{race_id}/wizard", response_model=List[schemas.Round])
def create_race_wizard(
    race_id: int, config: schemas.WizardConfiguration, db: Session = Depends(get_db)
):
    ...
    if existing_rounds:
        raise HTTPException(
            status_code=400,
            detail="Cannot use wizard: rounds already exist for this race.",
        )
    ...
    try:
        ...
        if config.general_round.type == "PACK":
            ...
            for den in dens:
                ...
                racers = (
                    db.query(models.Racer).filter(models.Racer.den_id == den.id).all()
                )
                if not racers:
                    continue
        ...
        for champ_cfg in config.championship_rounds:
            ...
            if champ_cfg.source == "DEN":
                den_count = (
                    db.query(models.Den).filter(models.Den.race_id == race_id).count()
                )
...
@app.get(
    "/races/{race_id}/rounds/{round_id}/advancement_status",
    response_model=schemas.AdvancementStatus,
)
def get_round_advancement_status(
    race_id: int, round_id: int, db: Session = Depends(get_db)
):
...
def advance_to_round(race_id: int, round_id: int, db: Session = Depends(get_db)):
    ...
    if not round_obj.advancement_source:
        raise HTTPException(
            status_code=400,
            detail="This round is not configured for automatic advancement.",
        )
    ...
    if not winner_ids:
        raise HTTPException(
            status_code=400,
            detail="No racers found to advance. Are previous rounds finished?",
        )
...
def bulk_auto_number_racers(
    request: schemas.BulkRacerActionRequest, db: Session = Depends(get_db)
):
    ...
    racer = (
        db.query(models.Racer).filter(models.Racer.id == request.racer_ids[0]).first()
    )
    if not racer:
        raise HTTPException(status_code=404, detail="Racer not found")
...
def bulk_move_to_den(
    request: schemas.BulkRacerMoveRequest, db: Session = Depends(get_db)
):
...
def create_round(
    race_id: int, round_data: schemas.RoundCreate, db: Session = Depends(get_db)
):
    # 1. Verification: ensure race exists
    race = db.query(models.Race).filter(models.Race.id == race_id).first()
    if not race:
    ...
    try:
        ...
        if round_data.advancement_source:
            ...
            if not general_exists:
                raise ValueError(
                    "Cannot add championship round: no general rounds exist yet. Schedule at least one general round first."
                )
        ...
        # 3. Case: General Round (DEN)
            ...
            for den in dens:
                ...
                round_obj = crud.create_round(
                    ...
                    next_round_number,
                    round_data.scheduling_strategy,
                    f"{den.name} - {round_data.name}" if round_data.name else den.name,
                )

                # Generate Heats with runs_per_lane
                for i in range(round_data.runs_per_lane):
                    crud.generate_heats_for_round(
                        db, round_obj.id, racer_ids=racer_ids, clear_existing=(i == 0)
                    )
...
def update_round(
    round_id: int, round_update: schemas.RoundUpdate, db: Session = Depends(get_db)
):
...
def reorder_heats(request: schemas.HeatReorderRequest, db: Session = Depends(get_db)):
    ...
    return schemas.HeatReorderResponse(
        updated_count=len(updated_heats), heats=updated_heats
    )
    ...
    if is_resetting:
        ...
        try:
            ...
            round_heats = (
                ...
                .filter(models.Heat.round_id == db_heat.round_id)
                .all()
            )
            if round_heats:
                ...
                if db_heat.heat_number == max_heat_num:
                    ...
                    round_obj = (
                        db.query(models.Round)
                        .filter(models.Round.id == db_heat.round_id)
                        .first()
                    )
                    if round_obj and not round_obj.advancement_source:
                        ...
                        if next_round:
                            print(
                                f"Heat reset detected on last heat of round {round_obj.id}. Reverting round {next_round.id}..."
                            )
...
def _check_and_advance_championship(db: Session, race_id: int):
    ...
    gen_rounds = (
        db.query(models.Round)
        .filter(
            models.Round.race_id == race_id, models.Round.advancement_source == None
        )
        .all()
    )
