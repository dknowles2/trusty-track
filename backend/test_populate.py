...
def test_populate_respects_per_den_numbering(db):
    ...
    den1 = crud.create_den(db, schemas.DenCreate(
        name="Lions",
        color="#F4D03F",
    ...
    den2 = crud.create_den(db, schemas.DenCreate(
        name="Tigers",
        color="#E67E22",
    ...
    for racer in racers:
        if racer.den_id == den1.id:
            assert 100 <= racer.car_number <= 199, f"Racer {racer.id} has car number {racer.car_number} outside den1 range"
        elif racer.den_id == den2.id:
            assert 200 <= racer.car_number <= 299, f"Racer {racer.id} has car number {racer.car_number} outside den2 range"
