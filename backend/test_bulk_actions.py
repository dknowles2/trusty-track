...
def test_bulk_operations(client, default_track):
    ...
    for i in range(5):
        r = client.post(
            "/racers/",
            json={"first_name": f"Racer{i}", "last_name": "Test", "race_id": race_id},
        ).json()
    ...
    den = client.post(
        f"/races/{race_id}/dens/", json={"name": "Bulk Den", "color": "#123456"}
    ).json()
    den_id = den["id"]

    resp = client.post(
        "/racers/bulk_move_to_den", json={"racer_ids": racer_ids, "den_id": den_id}
    )
    assert resp.status_code == 200
