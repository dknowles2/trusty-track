...
def test_rerun_last_heat_clears_next_round(client, default_track):
    ...
    client.post(
        ...
        json={"race_id": race_id, "round_number": 1, "name": "General"},
    )
    ...
    assert any(r["racer_id"] > 0 for r in first_heat_results), (
        "Championship round should be populated after general round finishes"
    )
    ...
    assert not has_actual_racers, (
        "Championship round should be cleared (reverted to placeholders) after rerunning last heat"
    )
