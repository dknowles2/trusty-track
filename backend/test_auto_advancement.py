...
# Setup test DB
SQLALCHEMY_DATABASE_URL = "sqlite:///./test_auto_advance.db"
...
def test_auto_advancement_with_placeholders(db):
    ...
    racers = []
    for i in range(4):
        r = models.Racer(first_name=f"R{i}", last_name=f"D{i}", race_id=race.id, den_id=den.id)
        db.add(r)
        racers.append(r)
    db.commit()
    ...
    rounds = db.query(models.Round).filter(models.Round.race_id == race.id).order_by(models.Round.round_number).all()
    assert len(rounds) == 2
    gen_round = rounds[0]
    champ_round = rounds[1]
    
    # Verify Champ round has placeholder heats
    champ_heats = db.query(models.Heat).filter(models.Heat.round_id == champ_round.id).all()
    ...
    db.expire_all()
    champ_heats_after = db.query(models.Heat).filter(models.Heat.round_id == champ_round.id).all()
