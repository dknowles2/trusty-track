...
test('the result controls fit above the fold at 1366×768 on a 4-lane no-timer track (#940)', async ({
    ...
    // The same per-attempt suffix `seedRace` in `support.ts` uses, and for
...
test("Home badges a race's schedule status as heats are generated and run (#847)", async ({ page }) => {
    ...
    const { raceId, racers, name } = await seedRace(page, 'Race Day Status Badge');

    await page.goto('/');
    // Exact, not `hasText`: under `--repeat-each` this name is a substring of
    // the sibling repetition's suffixed one, and both rows would match (#1320).
