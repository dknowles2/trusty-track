...
test('take screenshots', async ({ page }) => {
    ...
    // between the two shows "Check in cars" still reading "0 of 19" above a
    ...
    // racer is checked in, which one of nineteen never is here, so waiting
    // on `data-done` would hang. The count is the exact condition this wait
    // exists to settle, and reads correctly either way.
    await expect(page.getByTestId('setup-step-checkin')).toContainText('1 of 19 checked in');
