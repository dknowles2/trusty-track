...
function mockQueries({ races = [] as Array<{ id: number; name: string }>, installDefault = DEFAULT_TERMINOLOGY } = {}) {
    vi.mocked(useQuery).mockImplementation(((args: { query: { definitions: Array<{ name?: { value: string } }> }; pause?: boolean }) => {
        ...
        if (name === 'GetRaceSetupSource') {
            return [
                args.pause ? { data: undefined, fetching: false, stale: false } : { data: { race: lastYear }, fetching: false, stale: false },
                vi.fn(),
            ];
        }
